#region Declaim
//---------------------------------------------------------------------------
// Name:		QmisOutboundLogDAL
// Function: 	Expose data in table TI_IFM_QMIS_OUTBOUND_LOG from database as business object to MES system.
// Tool:		T4
...
{     
	//// <summary>
    /// QmisOutboundLogDAL对应表[TI_IFM_QMIS_OUTBOUND_LOG]
    /// </summary>
    public partial class QmisOutboundLogDAL : BusinessObjectProvider<QmisOutboundLogInfo>
	{
		#region Sql Statements
		private const string TI_IFM_QMIS_OUTBOUND_LOG_SELECT_BY_ID =
			@"SELECT ID,
				FID,
				TRANS_NO,
				SOURCE_SYSTEM,
				...
				MODIFY_USER,
				MODIFY_DATE				  
				FROM [LES].[TI_IFM_QMIS_OUTBOUND_LOG] WITH(NOLOCK) WHERE [VALID_FLAG] = 1  AND ID =@ID;";
			
		private const string TI_IFM_QMIS_OUTBOUND_LOG_SELECT = 
			@"SELECT ID,
				FID,
				TRANS_NO,
				SOURCE_SYSTEM,
				...
				MODIFY_USER,
				MODIFY_DATE				 
				FROM [LES].[TI_IFM_QMIS_OUTBOUND_LOG] WITH (NOLOCK) WHERE [VALID_FLAG] = 1 {0};";
		
		private const string TI_IFM_QMIS_OUTBOUND_LOG_SELECT_COUNTS = 
			@"SELECT count(*) FROM [LES].[TI_IFM_QMIS_OUTBOUND_LOG]  WITH(NOLOCK) WHERE [VALID_FLAG] = 1 {0};";

		private const string TI_IFM_QMIS_OUTBOUND_LOG_INSERT =
			@"INSERT INTO [LES].[TI_IFM_QMIS_OUTBOUND_LOG] (
				FID,
				TRANS_NO,
				SOURCE_SYSTEM,
				TARGET_SYSTEM,
		...
		private const string TI_IFM_QMIS_OUTBOUND_LOG_UPDATE =
			@"UPDATE [LES].[TI_IFM_QMIS_OUTBOUND_LOG] WITH(ROWLOCK) 
				SET FID=@FID,
				TRANS_NO=@TRANS_NO,
		...
		private const string TI_IFM_QMIS_OUTBOUND_LOG_DELETE =
			@"DELETE FROM [LES].[TI_IFM_QMIS_OUTBOUND_LOG] WITH(ROWLOCK)  
				WHERE [VALID_FLAG] = 1  AND ID =@ID;";
		#endregion
		...
		/// <summary>
		/// Get QmisOutboundLogInfo
		/// </summary>
		/// <param name="ID">QmisOutboundLogInfo Primary key </param>
		/// <returns></returns> 
		public QmisOutboundLogInfo GetInfo(long aId)
		{	
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TI_IFM_QMIS_OUTBOUND_LOG_SELECT_BY_ID);
			db.AddInParameter(dbCommand, "@ID", DbType.Int64, aId);
			using (IDataReader dr = db.ExecuteReader(dbCommand))
			{
				if (dr.Read())
					return CreateQmisOutboundLogInfo(dr);
			}
			return null;
		}
		/// <summary>
		...
		/// <returns>QmisOutboundLogInfo Collection </returns>
		public List<QmisOutboundLogInfo> GetList(string textWhere,string orderText)
		{
			...
			return GetList(string.Format(TI_IFM_QMIS_OUTBOUND_LOG_SELECT, query));
		}		
		/// <summary>
		/// Get data collection
		/// </summary>
		...
		/// <returns>QmisOutboundLogInfo Collection </returns>
		public List<QmisOutboundLogInfo> GetList(string sql)
		{
			Database db = DatabaseFactory.CreateDatabase();
        ...
        /// <returns></returns>
		public List<QmisOutboundLogInfo> GetListByPage(string textWhere, string textOrder, int pageIndex, int pageRow)
		{
        ...
        {			
			...
			Database db = DatabaseFactory.CreateDatabase();
            DbCommand cmd = db.GetSqlStringCommand(string.Format(TI_IFM_QMIS_OUTBOUND_LOG_SELECT_COUNTS, textWhere));
		...
		public long Add(QmisOutboundLogInfo info)
		{ 
			Database db = DatabaseFactory.CreateDatabase();	
			DbCommand dbCommand = db.GetSqlStringCommand(TI_IFM_QMIS_OUTBOUND_LOG_INSERT);			
		...
		/// </summary>
		/// <param name="info">info</param>
		public int Update(QmisOutboundLogInfo info)
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TI_IFM_QMIS_OUTBOUND_LOG_UPDATE);				
		...
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TI_IFM_QMIS_OUTBOUND_LOG_DELETE);
		    db.AddInParameter(dbCommand, "@ID", DbType.Int64, aId);
			return int.Parse("0" + db.ExecuteNonQuery(dbCommand));
		}/// <summary>
		/// LogciDelete
		...
		{
		    string sql = "update [LES].[TI_IFM_QMIS_OUTBOUND_LOG] WITH(ROWLOCK) "
		...
		private static QmisOutboundLogInfo CreateQmisOutboundLogInfo(IDataReader rdr)
		{
			QmisOutboundLogInfo info = new QmisOutboundLogInfo();
        ...
        public List<QmisOutboundLogInfo> GetListForUnsend()
        {
            string sql = "select [ID],[FID],[METHOD_CODE] "
                + "from [LES].[TI_IFM_QMIS_OUTBOUND_LOG] with(nolock) "
                + "where [VALID_FLAG] = 1 "
