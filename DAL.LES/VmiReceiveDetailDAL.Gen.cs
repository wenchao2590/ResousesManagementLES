#region Declaim
//---------------------------------------------------------------------------
// Name:		VmiReceiveDetailDAL
// Function: 	Expose data in table TT_WMM_VMI_RECEIVE_DETAIL from database as business object to MES system.
// Tool:		T4
...
{     
	//// <summary>
    /// VmiReceiveDetailDAL对应表[TT_WMM_VMI_RECEIVE_DETAIL]
    /// </summary>
    public partial class VmiReceiveDetailDAL : BusinessObjectProvider<VmiReceiveDetailInfo>
	{
		#region Sql Statements
		private const string TT_WMM_VMI_RECEIVE_DETAIL_SELECT_BY_ID =
			@"SELECT ID,
				FID,
				RECEIVE_FID,
				PLANT,
				...
				SUM_WEIGHT,
				SUM_VOLUME				  
				FROM [LES].[TT_WMM_VMI_RECEIVE_DETAIL] WITH(NOLOCK) WHERE [VALID_FLAG] = 1  AND ID =@ID;";
			
		private const string TT_WMM_VMI_RECEIVE_DETAIL_SELECT = 
			@"SELECT ID,
				FID,
				RECEIVE_FID,
				PLANT,
				...
				SUM_WEIGHT,
				SUM_VOLUME				 
				FROM [LES].[TT_WMM_VMI_RECEIVE_DETAIL] WITH (NOLOCK) WHERE [VALID_FLAG] = 1 {0};";
		
		private const string TT_WMM_VMI_RECEIVE_DETAIL_SELECT_COUNTS = 
			@"SELECT count(*) FROM [LES].[TT_WMM_VMI_RECEIVE_DETAIL]  WITH(NOLOCK) WHERE [VALID_FLAG] = 1 {0};";

		private const string TT_WMM_VMI_RECEIVE_DETAIL_INSERT =
		...
		private const string TT_WMM_VMI_RECEIVE_DETAIL_UPDATE =
			@"UPDATE [LES].[TT_WMM_VMI_RECEIVE_DETAIL] WITH(ROWLOCK) 
				SET FID=@FID,
		...
		private const string TT_WMM_VMI_RECEIVE_DETAIL_DELETE =
			@"DELETE FROM [LES].[TT_WMM_VMI_RECEIVE_DETAIL] WITH(ROWLOCK)  
				WHERE [VALID_FLAG] = 1  AND ID =@ID;";
		#endregion
		...
		/// <summary>
		/// Get VmiReceiveDetailInfo
		/// </summary>
		/// <param name="ID">VmiReceiveDetailInfo Primary key </param>
		/// <returns></returns> 
		public VmiReceiveDetailInfo GetInfo(long aId)
		{	
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TT_WMM_VMI_RECEIVE_DETAIL_SELECT_BY_ID);
			db.AddInParameter(dbCommand, "@ID", DbType.Int64, aId);
			using (IDataReader dr = db.ExecuteReader(dbCommand))
			{
				if (dr.Read())
					return CreateVmiReceiveDetailInfo(dr);
			}
			return null;
		}
		/// <summary>
		...
		/// <returns>VmiReceiveDetailInfo Collection </returns>
		public List<VmiReceiveDetailInfo> GetList(string textWhere,string orderText)
		{
			...
			return GetList(string.Format(TT_WMM_VMI_RECEIVE_DETAIL_SELECT, query));
		}		
		/// <summary>
		/// Get data collection
		/// </summary>
		...
		/// <returns>VmiReceiveDetailInfo Collection </returns>
		public List<VmiReceiveDetailInfo> GetList(string sql)
		{
			Database db = DatabaseFactory.CreateDatabase();
			...
			using (IDataReader dr = db.ExecuteReader(dbCommand))
			{
				while (dr.Read())
				{
					list.Add(CreateVmiReceiveDetailInfo(dr));
				}
			}
			return list;
		}		
        ...
        /// <returns></returns>
		public List<VmiReceiveDetailInfo> GetListByPage(string textWhere, string textOrder, int pageIndex, int pageRow)
		{
            ...
            string sql = "select top " + pageRow + " * from "
                ...
                + ",* from [LES].[TT_WMM_VMI_RECEIVE_DETAIL]  WITH(NOLOCK) " + whereText + ") T "
        ...
        {			
			...
			Database db = DatabaseFactory.CreateDatabase();
            DbCommand cmd = db.GetSqlStringCommand(string.Format(TT_WMM_VMI_RECEIVE_DETAIL_SELECT_COUNTS, textWhere));
		...
		public long Add(VmiReceiveDetailInfo info)
		{ 
			Database db = DatabaseFactory.CreateDatabase();	
			DbCommand dbCommand = db.GetSqlStringCommand(TT_WMM_VMI_RECEIVE_DETAIL_INSERT);			
		...
		/// </summary>
		/// <param name="info"> info</param>
		public static string GetInsertSql(VmiReceiveDetailInfo info)
		{
			return  
			@"insert into [LES].[TT_WMM_VMI_RECEIVE_DETAIL] (
				FID,
		...
		/// </summary>
		/// <param name="info">info</param>
		public int Update(VmiReceiveDetailInfo info)
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TT_WMM_VMI_RECEIVE_DETAIL_UPDATE);				
		...
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TT_WMM_VMI_RECEIVE_DETAIL_DELETE);
		    db.AddInParameter(dbCommand, "@ID", DbType.Int64, aId);
			return int.Parse("0" + db.ExecuteNonQuery(dbCommand));
		}/// <summary>
		/// LogciDelete
		...
		{
		    string sql = "update [LES].[TT_WMM_VMI_RECEIVE_DETAIL] WITH(ROWLOCK) "
		...
		private static VmiReceiveDetailInfo CreateVmiReceiveDetailInfo(IDataReader rdr)
		{
