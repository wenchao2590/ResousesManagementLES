#region Declaim
//---------------------------------------------------------------------------
// Name:		WmsVmiJisPullOrderDAL
// Function: 	Expose data in table TI_IFM_WMS_VMI_JIS_PULL_ORDER from database as business object to MES system.
// Tool:		T4
...
{     
	//// <summary>
    /// WmsVmiJisPullOrderDAL对应表[TI_IFM_WMS_VMI_JIS_PULL_ORDER]
    /// </summary>
    public partial class WmsVmiJisPullOrderDAL : BusinessObjectProvider<WmsVmiJisPullOrderInfo>
	{
		#region Sql Statements
		private const string TI_IFM_WMS_VMI_JIS_PULL_ORDER_SELECT_BY_ID =
			@"SELECT ID,
				FID,
				LOG_FID,
				PLANT,
				...
				MODIFY_USER,
				COMMENTS				  
				FROM [LES].[TI_IFM_WMS_VMI_JIS_PULL_ORDER] WITH(NOLOCK) WHERE [VALID_FLAG] = 1  AND ID =@ID;";
			
		private const string TI_IFM_WMS_VMI_JIS_PULL_ORDER_SELECT = 
			@"SELECT ID,
				FID,
				LOG_FID,
				PLANT,
				...
				MODIFY_USER,
				COMMENTS				 
				FROM [LES].[TI_IFM_WMS_VMI_JIS_PULL_ORDER] WITH (NOLOCK) WHERE [VALID_FLAG] = 1 {0};";
		
		private const string TI_IFM_WMS_VMI_JIS_PULL_ORDER_SELECT_COUNTS = 
			@"SELECT count(*) FROM [LES].[TI_IFM_WMS_VMI_JIS_PULL_ORDER]  WITH(NOLOCK) WHERE [VALID_FLAG] = 1 {0};";

		private const string TI_IFM_WMS_VMI_JIS_PULL_ORDER_INSERT =
			@"INSERT INTO [LES].[TI_IFM_WMS_VMI_JIS_PULL_ORDER] (
				FID,
				LOG_FID,
				PLANT,
				ORDER_CODE,
		...
		private const string TI_IFM_WMS_VMI_JIS_PULL_ORDER_UPDATE =
			@"UPDATE [LES].[TI_IFM_WMS_VMI_JIS_PULL_ORDER] WITH(ROWLOCK) 
				SET FID=@FID,
				LOG_FID=@LOG_FID,
				PLANT=@PLANT,
				ORDER_CODE=@ORDER_CODE,
				DOCK=@DOCK,
				SEQUENCENUMBE=@SEQUENCENUMBE,
		...
		private const string TI_IFM_WMS_VMI_JIS_PULL_ORDER_DELETE =
			@"DELETE FROM [LES].[TI_IFM_WMS_VMI_JIS_PULL_ORDER] WITH(ROWLOCK)  
				WHERE [VALID_FLAG] = 1  AND ID =@ID;";
		#endregion
		...
		/// <summary>
		/// Get WmsVmiJisPullOrderInfo
		/// </summary>
		/// <param name="ID">WmsVmiJisPullOrderInfo Primary key </param>
		/// <returns></returns> 
		public WmsVmiJisPullOrderInfo GetInfo(long aId)
		{	
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TI_IFM_WMS_VMI_JIS_PULL_ORDER_SELECT_BY_ID);
			db.AddInParameter(dbCommand, "@ID", DbType.Int64, aId);
			using (IDataReader dr = db.ExecuteReader(dbCommand))
			{
				if (dr.Read())
					return CreateWmsVmiJisPullOrderInfo(dr);
			}
			return null;
		}
		/// <summary>
		...
		/// <returns>WmsVmiJisPullOrderInfo Collection </returns>
		public List<WmsVmiJisPullOrderInfo> GetList(string textWhere,string orderText)
		{
			...
			return GetList(string.Format(TI_IFM_WMS_VMI_JIS_PULL_ORDER_SELECT, query));
		}		
		/// <summary>
		/// Get data collection
		/// </summary>
		...
		/// <returns>WmsVmiJisPullOrderInfo Collection </returns>
		public List<WmsVmiJisPullOrderInfo> GetList(string sql)
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(sql);
			List<WmsVmiJisPullOrderInfo> list = new List<WmsVmiJisPullOrderInfo>();
			using (IDataReader dr = db.ExecuteReader(dbCommand))
			{
				while (dr.Read())
				{
					list.Add(CreateWmsVmiJisPullOrderInfo(dr));
				}
			}
			return list;
		}		
        ...
        /// <returns></returns>
		public List<WmsVmiJisPullOrderInfo> GetListByPage(string textWhere, string textOrder, int pageIndex, int pageRow)
		{
            ...
            string sql = "select top " + pageRow + " * from "
                ...
                + ",* from [LES].[TI_IFM_WMS_VMI_JIS_PULL_ORDER]  WITH(NOLOCK) " + whereText + ") T "
        ...
        {			
			...
			Database db = DatabaseFactory.CreateDatabase();
            DbCommand cmd = db.GetSqlStringCommand(string.Format(TI_IFM_WMS_VMI_JIS_PULL_ORDER_SELECT_COUNTS, textWhere));
		...
		public long Add(WmsVmiJisPullOrderInfo info)
		{ 
			Database db = DatabaseFactory.CreateDatabase();	
			DbCommand dbCommand = db.GetSqlStringCommand(TI_IFM_WMS_VMI_JIS_PULL_ORDER_INSERT);			
			...
			db.AddInParameter(dbCommand, "@SEQUENCENUMBE", DbType.String, info.Sequencenumbe);
			...
			db.AddInParameter(dbCommand, "@START_INFOPOIN_TTIME", DbType.DateTime, info.StartInfopoinTtime);
			...
			db.AddInParameter(dbCommand, "@END_VEHICLESEQ_NO", DbType.Int32, info.EndVehicleseqNo);
		...
		/// </summary>
		/// <param name="info"> info</param>
		public static string GetInsertSql(WmsVmiJisPullOrderInfo info)
		{
			return  
			@"insert into [LES].[TI_IFM_WMS_VMI_JIS_PULL_ORDER] (
				FID,
				...
				(string.IsNullOrEmpty(info.Sequencenumbe) ? "NULL" : "N'" + info.Sequencenumbe + "'") + ","+
		...
		/// </summary>
		/// <param name="info">info</param>
		public int Update(WmsVmiJisPullOrderInfo info)
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TI_IFM_WMS_VMI_JIS_PULL_ORDER_UPDATE);				
		...
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TI_IFM_WMS_VMI_JIS_PULL_ORDER_DELETE);
		    db.AddInParameter(dbCommand, "@ID", DbType.Int64, aId);
			return int.Parse("0" + db.ExecuteNonQuery(dbCommand));
		}/// <summary>
		/// LogciDelete
		...
		{
		    string sql = "update [LES].[TI_IFM_WMS_VMI_JIS_PULL_ORDER] WITH(ROWLOCK) "
		...
		private static WmsVmiJisPullOrderInfo CreateWmsVmiJisPullOrderInfo(IDataReader rdr)
		{
			...
			info.Sequencenumbe = DBConvert.GetString(rdr, rdr.GetOrdinal("SEQUENCENUMBE"));			
			...
			info.StartInfopoinTtime = DBConvert.GetDateTimeNullable(rdr, rdr.GetOrdinal("START_INFOPOIN_TTIME"));			
			...
			info.EndVehicleseqNo = DBConvert.GetInt32Nullable(rdr, rdr.GetOrdinal("END_VEHICLESEQ_NO"));			
