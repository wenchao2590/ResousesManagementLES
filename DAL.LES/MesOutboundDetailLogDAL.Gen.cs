#region Declaim
//---------------------------------------------------------------------------
// Name:		MesOutboundDetailLogDAL
// Function: 	Expose data in table TI_IFM_MES_OUTBOUND_DETAIL_LOG from database as business object to MES system.
// Tool:		T4
...
{     
	//// <summary>
    /// MesOutboundDetailLogDAL对应表[TI_IFM_MES_OUTBOUND_DETAIL_LOG]
    /// </summary>
    public partial class MesOutboundDetailLogDAL : BusinessObjectProvider<MesOutboundDetailLogInfo>
	{
		#region Sql Statements
		private const string TI_IFM_MES_OUTBOUND_DETAIL_LOG_SELECT_BY_ID =
			@"SELECT ID,
				FID,
				LOG_FID,
				TRANS_NO,
				...
				MODIFY_DATE,
				VALID_FLAG				  
				FROM [LES].[TI_IFM_MES_OUTBOUND_DETAIL_LOG] WITH(NOLOCK) WHERE [VALID_FLAG] = 1  AND ID =@ID;";
			
		private const string TI_IFM_MES_OUTBOUND_DETAIL_LOG_SELECT = 
			@"SELECT ID,
				FID,
				LOG_FID,
				TRANS_NO,
				...
				MODIFY_DATE,
				VALID_FLAG				 
				FROM [LES].[TI_IFM_MES_OUTBOUND_DETAIL_LOG] WITH (NOLOCK) WHERE [VALID_FLAG] = 1 {0};";
		
		private const string TI_IFM_MES_OUTBOUND_DETAIL_LOG_SELECT_COUNTS = 
			@"SELECT count(*) FROM [LES].[TI_IFM_MES_OUTBOUND_DETAIL_LOG]  WITH(NOLOCK) WHERE [VALID_FLAG] = 1 {0};";

		private const string TI_IFM_MES_OUTBOUND_DETAIL_LOG_INSERT =
			@"INSERT INTO [LES].[TI_IFM_MES_OUTBOUND_DETAIL_LOG] (
				FID,
				LOG_FID,
				TRANS_NO,
				SOURCE_SYSTEM,
		...
		private const string TI_IFM_MES_OUTBOUND_DETAIL_LOG_UPDATE =
			@"UPDATE [LES].[TI_IFM_MES_OUTBOUND_DETAIL_LOG] WITH(ROWLOCK) 
				SET FID=@FID,
				LOG_FID=@LOG_FID,
				TRANS_NO=@TRANS_NO,
		...
		private const string TI_IFM_MES_OUTBOUND_DETAIL_LOG_DELETE =
			@"DELETE FROM [LES].[TI_IFM_MES_OUTBOUND_DETAIL_LOG] WITH(ROWLOCK)  
				WHERE [VALID_FLAG] = 1  AND ID =@ID;";
		#endregion
		...
		/// <summary>
		/// Get MesOutboundDetailLogInfo
		/// </summary>
		/// <param name="ID">MesOutboundDetailLogInfo Primary key </param>
		/// <returns></returns> 
		public MesOutboundDetailLogInfo GetInfo(int aId)
		{	
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TI_IFM_MES_OUTBOUND_DETAIL_LOG_SELECT_BY_ID);
			db.AddInParameter(dbCommand, "@ID", DbType.Int32, aId);
			using (IDataReader dr = db.ExecuteReader(dbCommand))
			{
				if (dr.Read())
					return CreateMesOutboundDetailLogInfo(dr);
			}
			return null;
		}
		/// <summary>
		...
		/// <returns>MesOutboundDetailLogInfo Collection </returns>
		public List<MesOutboundDetailLogInfo> GetList(string textWhere,string orderText)
		{
			...
			return GetList(string.Format(TI_IFM_MES_OUTBOUND_DETAIL_LOG_SELECT, query));
		}		
		/// <summary>
		/// Get data collection
		/// </summary>
		...
		/// <returns>MesOutboundDetailLogInfo Collection </returns>
		public List<MesOutboundDetailLogInfo> GetList(string sql)
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(sql);
			List<MesOutboundDetailLogInfo> list = new List<MesOutboundDetailLogInfo>();
			using (IDataReader dr = db.ExecuteReader(dbCommand))
			{
				while (dr.Read())
				{
					list.Add(CreateMesOutboundDetailLogInfo(dr));
				}
			}
			return list;
		}		
        ...
        /// <returns></returns>
		public List<MesOutboundDetailLogInfo> GetListByPage(string textWhere, string textOrder, int pageIndex, int pageRow)
		{
            ...
            string sql = "select top " + pageRow + " * from "
                ...
                + ",* from [LES].[TI_IFM_MES_OUTBOUND_DETAIL_LOG]  WITH(NOLOCK) " + whereText + ") T "
        ...
        {			
			...
			Database db = DatabaseFactory.CreateDatabase();
            DbCommand cmd = db.GetSqlStringCommand(string.Format(TI_IFM_MES_OUTBOUND_DETAIL_LOG_SELECT_COUNTS, textWhere));
		...
		public int Add(MesOutboundDetailLogInfo info)
		{ 
			Database db = DatabaseFactory.CreateDatabase();	
			DbCommand dbCommand = db.GetSqlStringCommand(TI_IFM_MES_OUTBOUND_DETAIL_LOG_INSERT);			
		...
		/// </summary>
		/// <param name="info">info</param>
		public int Update(MesOutboundDetailLogInfo info)
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TI_IFM_MES_OUTBOUND_DETAIL_LOG_UPDATE);				
		...
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TI_IFM_MES_OUTBOUND_DETAIL_LOG_DELETE);
		    db.AddInParameter(dbCommand, "@ID", DbType.Int32, aId);
			return int.Parse("0" + db.ExecuteNonQuery(dbCommand));
		}/// <summary>
		/// LogciDelete
		...
		{
		    string sql = "update [LES].[TI_IFM_MES_OUTBOUND_DETAIL_LOG] WITH(ROWLOCK) "
