#region Declaim
//---------------------------------------------------------------------------
// Name:		PcsPullOrderDAL
// Function: 	Expose data in table TT_MPM_PCS_PULL_ORDER from database as business object to MES system.
// Tool:		T4
...
{     
	//// <summary>
    /// PcsPullOrderDAL对应表[TT_MPM_PCS_PULL_ORDER]
    /// </summary>
    public partial class PcsPullOrderDAL : BusinessObjectProvider<PcsPullOrderInfo>
	{
		#region Sql Statements
		private const string TT_MPM_PCS_PULL_ORDER_SELECT_BY_ID =
			@"SELECT ID,
				FID,
				ORDER_CODE,
				PART_BOX_CODE,
				...
				MODIFY_DATE,
				MODIFY_USER				  
				FROM [LES].[TT_MPM_PCS_PULL_ORDER] WITH(NOLOCK) WHERE [VALID_FLAG] = 1  AND ID =@ID;";
			
		private const string TT_MPM_PCS_PULL_ORDER_SELECT = 
			@"SELECT ID,
				FID,
				ORDER_CODE,
				PART_BOX_CODE,
				...
				MODIFY_DATE,
				MODIFY_USER				 
				FROM [LES].[TT_MPM_PCS_PULL_ORDER] WITH (NOLOCK) WHERE [VALID_FLAG] = 1 {0};";
		
		private const string TT_MPM_PCS_PULL_ORDER_SELECT_COUNTS = 
			@"SELECT count(*) FROM [LES].[TT_MPM_PCS_PULL_ORDER]  WITH(NOLOCK) WHERE [VALID_FLAG] = 1 {0};";

		private const string TT_MPM_PCS_PULL_ORDER_INSERT =
			@"INSERT INTO [LES].[TT_MPM_PCS_PULL_ORDER] (
				FID,
				ORDER_CODE,
				PART_BOX_CODE,
				PART_BOX_NAME,
		...
		private const string TT_MPM_PCS_PULL_ORDER_UPDATE =
			@"UPDATE [LES].[TT_MPM_PCS_PULL_ORDER] WITH(ROWLOCK) 
				SET FID=@FID,
		...
		private const string TT_MPM_PCS_PULL_ORDER_DELETE =
			@"DELETE FROM [LES].[TT_MPM_PCS_PULL_ORDER] WITH(ROWLOCK)  
				WHERE [VALID_FLAG] = 1  AND ID =@ID;";
		#endregion
		...
		/// <summary>
		/// Get PcsPullOrderInfo
		/// </summary>
		/// <param name="ID">PcsPullOrderInfo Primary key </param>
		/// <returns></returns> 
		public PcsPullOrderInfo GetInfo(long aId)
		{	
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TT_MPM_PCS_PULL_ORDER_SELECT_BY_ID);
			db.AddInParameter(dbCommand, "@ID", DbType.Int64, aId);
			using (IDataReader dr = db.ExecuteReader(dbCommand))
			{
				if (dr.Read())
					return CreatePcsPullOrderInfo(dr);
			}
			return null;
		}
		/// <summary>
		...
		/// <returns>PcsPullOrderInfo Collection </returns>
		public List<PcsPullOrderInfo> GetList(string textWhere,string orderText)
		{
			...
			return GetList(string.Format(TT_MPM_PCS_PULL_ORDER_SELECT, query));
		}		
		/// <summary>
		/// Get data collection
		/// </summary>
		...
		/// <returns>PcsPullOrderInfo Collection </returns>
		public List<PcsPullOrderInfo> GetList(string sql)
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(sql);
			List<PcsPullOrderInfo> list = new List<PcsPullOrderInfo>();
			using (IDataReader dr = db.ExecuteReader(dbCommand))
			{
				while (dr.Read())
				{
					list.Add(CreatePcsPullOrderInfo(dr));
				}
			}
			return list;
		}		
        ...
        /// <returns></returns>
		public List<PcsPullOrderInfo> GetListByPage(string textWhere, string textOrder, int pageIndex, int pageRow)
		{
            ...
            string sql = "select top " + pageRow + " * from "
                ...
                + ",* from [LES].[TT_MPM_PCS_PULL_ORDER]  WITH(NOLOCK) " + whereText + ") T "
        ...
        {			
			...
			Database db = DatabaseFactory.CreateDatabase();
            DbCommand cmd = db.GetSqlStringCommand(string.Format(TT_MPM_PCS_PULL_ORDER_SELECT_COUNTS, textWhere));
		...
		public long Add(PcsPullOrderInfo info)
		{ 
			Database db = DatabaseFactory.CreateDatabase();	
			DbCommand dbCommand = db.GetSqlStringCommand(TT_MPM_PCS_PULL_ORDER_INSERT);			
		...
		/// </summary>
		/// <param name="info"> info</param>
		public static string GetInsertSql(PcsPullOrderInfo info)
		{
			return  
			@"insert into [LES].[TT_MPM_PCS_PULL_ORDER] (
				FID,
		...
		/// </summary>
		/// <param name="info">info</param>
		public int Update(PcsPullOrderInfo info)
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TT_MPM_PCS_PULL_ORDER_UPDATE);				
		...
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TT_MPM_PCS_PULL_ORDER_DELETE);
		    db.AddInParameter(dbCommand, "@ID", DbType.Int64, aId);
			return int.Parse("0" + db.ExecuteNonQuery(dbCommand));
		}/// <summary>
		/// LogciDelete
		...
		{
		    string sql = "update [LES].[TT_MPM_PCS_PULL_ORDER] WITH(ROWLOCK) "
		...
		private static PcsPullOrderInfo CreatePcsPullOrderInfo(IDataReader rdr)
		{
