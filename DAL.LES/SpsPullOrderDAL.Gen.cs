#region Declaim
//---------------------------------------------------------------------------
// Name:		SpsPullOrderDAL
// Function: 	Expose data in table TT_MPM_SPS_PULL_ORDER from database as business object to MES system.
// Tool:		T4
...
{     
	//// <summary>
    /// SpsPullOrderDAL对应表[TT_MPM_SPS_PULL_ORDER]
    /// </summary>
    public partial class SpsPullOrderDAL : BusinessObjectProvider<SpsPullOrderInfo>
	{
		#region Sql Statements
		private const string TT_MPM_SPS_PULL_ORDER_SELECT_BY_ID =
			@"SELECT ID,
				FID,
				ORDER_CODE,
				PART_BOX_CODE,
				...
				MODIFY_DATE,
				MODIFY_USER				  
				FROM [LES].[TT_MPM_SPS_PULL_ORDER] WITH(NOLOCK) WHERE [VALID_FLAG] = 1  AND ID =@ID;";
			
		private const string TT_MPM_SPS_PULL_ORDER_SELECT = 
			@"SELECT ID,
				FID,
				ORDER_CODE,
				PART_BOX_CODE,
				...
				MODIFY_DATE,
				MODIFY_USER				 
				FROM [LES].[TT_MPM_SPS_PULL_ORDER] WITH (NOLOCK) WHERE [VALID_FLAG] = 1 {0};";
		
		private const string TT_MPM_SPS_PULL_ORDER_SELECT_COUNTS = 
			@"SELECT count(*) FROM [LES].[TT_MPM_SPS_PULL_ORDER]  WITH(NOLOCK) WHERE [VALID_FLAG] = 1 {0};";

		private const string TT_MPM_SPS_PULL_ORDER_INSERT =
			@"INSERT INTO [LES].[TT_MPM_SPS_PULL_ORDER] (
				FID,
				ORDER_CODE,
				PART_BOX_CODE,
				PART_BOX_NAME,
		...
		private const string TT_MPM_SPS_PULL_ORDER_UPDATE =
			@"UPDATE [LES].[TT_MPM_SPS_PULL_ORDER] WITH(ROWLOCK) 
				SET FID=@FID,
		...
		private const string TT_MPM_SPS_PULL_ORDER_DELETE =
			@"DELETE FROM [LES].[TT_MPM_SPS_PULL_ORDER] WITH(ROWLOCK)  
				WHERE [VALID_FLAG] = 1  AND ID =@ID;";
		#endregion
		...
		/// <summary>
		/// Get SpsPullOrderInfo
		/// </summary>
		/// <param name="ID">SpsPullOrderInfo Primary key </param>
		/// <returns></returns> 
		public SpsPullOrderInfo GetInfo(long aId)
		{	
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TT_MPM_SPS_PULL_ORDER_SELECT_BY_ID);
			db.AddInParameter(dbCommand, "@ID", DbType.Int64, aId);
			using (IDataReader dr = db.ExecuteReader(dbCommand))
			{
				if (dr.Read())
					return CreateSpsPullOrderInfo(dr);
			}
			return null;
		}
		/// <summary>
		...
		/// <returns>SpsPullOrderInfo Collection </returns>
		public List<SpsPullOrderInfo> GetList(string textWhere,string orderText)
		{
			...
			return GetList(string.Format(TT_MPM_SPS_PULL_ORDER_SELECT, query));
		}		
		/// <summary>
		/// Get data collection
		/// </summary>
		...
		/// <returns>SpsPullOrderInfo Collection </returns>
		public List<SpsPullOrderInfo> GetList(string sql)
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(sql);
			List<SpsPullOrderInfo> list = new List<SpsPullOrderInfo>();
			using (IDataReader dr = db.ExecuteReader(dbCommand))
			{
				while (dr.Read())
				{
					list.Add(CreateSpsPullOrderInfo(dr));
				}
			}
			return list;
		}		
        ...
        /// <returns></returns>
		public List<SpsPullOrderInfo> GetListByPage(string textWhere, string textOrder, int pageIndex, int pageRow)
		{
            ...
            string sql = "select top " + pageRow + " * from "
                ...
                + ",* from [LES].[TT_MPM_SPS_PULL_ORDER]  WITH(NOLOCK) " + whereText + ") T "
        ...
        {			
			...
			Database db = DatabaseFactory.CreateDatabase();
            DbCommand cmd = db.GetSqlStringCommand(string.Format(TT_MPM_SPS_PULL_ORDER_SELECT_COUNTS, textWhere));
		...
		public long Add(SpsPullOrderInfo info)
		{ 
			Database db = DatabaseFactory.CreateDatabase();	
			DbCommand dbCommand = db.GetSqlStringCommand(TT_MPM_SPS_PULL_ORDER_INSERT);			
		...
		/// </summary>
		/// <param name="info"> info</param>
		public static string GetInsertSql(SpsPullOrderInfo info)
		{
			return  
			@"insert into [LES].[TT_MPM_SPS_PULL_ORDER] (
				FID,
		...
		/// </summary>
		/// <param name="info">info</param>
		public int Update(SpsPullOrderInfo info)
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TT_MPM_SPS_PULL_ORDER_UPDATE);				
		...
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TT_MPM_SPS_PULL_ORDER_DELETE);
		    db.AddInParameter(dbCommand, "@ID", DbType.Int64, aId);
			return int.Parse("0" + db.ExecuteNonQuery(dbCommand));
		}/// <summary>
		/// LogciDelete
		...
		{
		    string sql = "update [LES].[TT_MPM_SPS_PULL_ORDER] WITH(ROWLOCK) "
		...
		private static SpsPullOrderInfo CreateSpsPullOrderInfo(IDataReader rdr)
		{
