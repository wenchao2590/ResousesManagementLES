#region Declaim
//---------------------------------------------------------------------------
// Name:		BusinessExpenseOutDAL
// Function: 	Expose data in table TT_FIM_BUSINESS_EXPENSE_OUT from database as business object to MES system.
// Tool:		T4
...
{     
	//// <summary>
    /// BusinessExpenseOutDAL对应表[TT_FIM_BUSINESS_EXPENSE_OUT]
    /// </summary>
    public partial class BusinessExpenseOutDAL : BusinessObjectProvider<BusinessExpenseOutInfo>
	{
		#region Sql Statements
		private const string TT_FIM_BUSINESS_EXPENSE_OUT_SELECT_BY_ID =
			@"SELECT SUPPLIER_NUM,
				SUPPLIER_NAME,
				ORDER_NO,
				REIMBURSE_USER,
				...
				FID,
				CREATE_DATE				  
				FROM [LES].[TT_FIM_BUSINESS_EXPENSE_OUT] WITH(NOLOCK) WHERE [VALID_FLAG] = 1  AND ID =@ID;";
			
		private const string TT_FIM_BUSINESS_EXPENSE_OUT_SELECT = 
			@"SELECT SUPPLIER_NUM,
				SUPPLIER_NAME,
				ORDER_NO,
				REIMBURSE_USER,
				...
				FID,
				CREATE_DATE				 
				FROM [LES].[TT_FIM_BUSINESS_EXPENSE_OUT] WITH (NOLOCK) WHERE [VALID_FLAG] = 1 {0};";
		
		private const string TT_FIM_BUSINESS_EXPENSE_OUT_SELECT_COUNTS = 
			@"SELECT count(*) FROM [LES].[TT_FIM_BUSINESS_EXPENSE_OUT]  WITH(NOLOCK) WHERE [VALID_FLAG] = 1 {0};";

		private const string TT_FIM_BUSINESS_EXPENSE_OUT_INSERT =
			@"INSERT INTO [LES].[TT_FIM_BUSINESS_EXPENSE_OUT] (
				SUPPLIER_NUM,
				SUPPLIER_NAME,
				ORDER_NO,
				REIMBURSE_USER,
		...
		private const string TT_FIM_BUSINESS_EXPENSE_OUT_UPDATE =
			@"UPDATE [LES].[TT_FIM_BUSINESS_EXPENSE_OUT] WITH(ROWLOCK) 
		...
		private const string TT_FIM_BUSINESS_EXPENSE_OUT_DELETE =
			@"DELETE FROM [LES].[TT_FIM_BUSINESS_EXPENSE_OUT] WITH(ROWLOCK)  
				WHERE [VALID_FLAG] = 1  AND ID =@ID;";
		#endregion
		...
		/// <summary>
		/// Get BusinessExpenseOutInfo
		/// </summary>
		/// <param name="ID">BusinessExpenseOutInfo Primary key </param>
		/// <returns></returns> 
		public BusinessExpenseOutInfo GetInfo(long aId)
		{	
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TT_FIM_BUSINESS_EXPENSE_OUT_SELECT_BY_ID);
			db.AddInParameter(dbCommand, "@ID", DbType.Int64, aId);
			using (IDataReader dr = db.ExecuteReader(dbCommand))
			{
				if (dr.Read())
					return CreateBusinessExpenseOutInfo(dr);
			}
			return null;
		}
		/// <summary>
		...
		/// <returns>BusinessExpenseOutInfo Collection </returns>
		public List<BusinessExpenseOutInfo> GetList(string textWhere,string orderText)
		{
			...
			return GetList(string.Format(TT_FIM_BUSINESS_EXPENSE_OUT_SELECT, query));
		}		
		/// <summary>
		/// Get data collection
		/// </summary>
		...
		/// <returns>BusinessExpenseOutInfo Collection </returns>
		public List<BusinessExpenseOutInfo> GetList(string sql)
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(sql);
			List<BusinessExpenseOutInfo> list = new List<BusinessExpenseOutInfo>();
			using (IDataReader dr = db.ExecuteReader(dbCommand))
			{
				while (dr.Read())
				{
					list.Add(CreateBusinessExpenseOutInfo(dr));
				}
			}
			return list;
		}		
        ...
        /// <returns></returns>
		public List<BusinessExpenseOutInfo> GetListByPage(string textWhere, string textOrder, int pageIndex, int pageRow)
		{
            ...
            string sql = "select top " + pageRow + " * from "
                ...
                + ",* from [LES].[TT_FIM_BUSINESS_EXPENSE_OUT]  WITH(NOLOCK) " + whereText + ") T "
        ...
        {			
			...
			Database db = DatabaseFactory.CreateDatabase();
            DbCommand cmd = db.GetSqlStringCommand(string.Format(TT_FIM_BUSINESS_EXPENSE_OUT_SELECT_COUNTS, textWhere));
		...
		public long Add(BusinessExpenseOutInfo info)
		{ 
			Database db = DatabaseFactory.CreateDatabase();	
			DbCommand dbCommand = db.GetSqlStringCommand(TT_FIM_BUSINESS_EXPENSE_OUT_INSERT);			
			...
			db.AddInParameter(dbCommand, "@CHECK_NO", DbType.String, info.CheckNo);
		...
		/// </summary>
		/// <param name="info">info</param>
		public int Update(BusinessExpenseOutInfo info)
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TT_FIM_BUSINESS_EXPENSE_OUT_UPDATE);				
			db.AddInParameter(dbCommand, "@SUPPLIER_NUM", DbType.String, info.SupplierNum);
			db.AddInParameter(dbCommand, "@SUPPLIER_NAME", DbType.String, info.SupplierName);
			db.AddInParameter(dbCommand, "@ORDER_NO", DbType.String, info.OrderNo);
			db.AddInParameter(dbCommand, "@REIMBURSE_USER", DbType.String, info.ReimburseUser);
		...
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TT_FIM_BUSINESS_EXPENSE_OUT_DELETE);
		    db.AddInParameter(dbCommand, "@ID", DbType.Int64, aId);
			return int.Parse("0" + db.ExecuteNonQuery(dbCommand));
		}/// <summary>
		/// LogciDelete
		...
		{
		    string sql = "update [LES].[TT_FIM_BUSINESS_EXPENSE_OUT] WITH(ROWLOCK) "
     ...
     db.AddInParameter(dbCommand, "@VALID_FLAG", DbType.Boolean, false);
		...
		private static BusinessExpenseOutInfo CreateBusinessExpenseOutInfo(IDataReader rdr)
		{
			...
			info.CheckNo = DBConvert.GetString(rdr, rdr.GetOrdinal("CHECK_NO"));			
