...
//---------------------------------------------------------------------------
// Name:		Shgx004DAL
// Function: 	Expose data in table V_WMM_SHGX_004 from database as business object to MES system.
// Tool:		T4
...
{     
	//// <summary>
    /// Shgx004DAL对应表[V_WMM_SHGX_004]
    /// </summary>
    public partial class Shgx004DAL : BusinessObjectProvider<Shgx004Info>
	{
		#region Sql Statements
		private const string V_WMM_SHGX_004_SELECT_BY_ID =
			"";
			
		private const string V_WMM_SHGX_004_SELECT = 
			@"SELECT CustTrustNo,
				CustName,
				Ysf,
				Qtfy,
				...
				TargetZone,
				Jcf				 
				FROM [LES].[V_WMM_SHGX_004] WITH (NOLOCK) WHERE 1=1 {0};";
		
		private const string V_WMM_SHGX_004_SELECT_COUNTS = 
			@"SELECT count(*) FROM [LES].[V_WMM_SHGX_004]  WITH(NOLOCK) WHERE 1=1 {0};";

		private const string V_WMM_SHGX_004_INSERT =
			@"INSERT INTO [LES].[V_WMM_SHGX_004] (
				CustTrustNo,
				CustName,
				Ysf,
				Qtfy,
		...
		private const string V_WMM_SHGX_004_UPDATE =
			"";

		private const string V_WMM_SHGX_004_DELETE =
			"";
		#endregion
		...
		/// <returns>Shgx004Info Collection </returns>
		public List<Shgx004Info> GetList(string textWhere,string orderText)
		{
			...
			return GetList(string.Format(V_WMM_SHGX_004_SELECT, query));
		}		
		/// <summary>
		/// Get data collection
		/// </summary>
		...
		/// <returns>Shgx004Info Collection </returns>
		public List<Shgx004Info> GetList(string sql)
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(sql);
			List<Shgx004Info> list = new List<Shgx004Info>();
			using (IDataReader dr = db.ExecuteReader(dbCommand))
			{
				while (dr.Read())
				{
					list.Add(CreateShgx004Info(dr));
				}
			}
			return list;
		}		
        ...
        /// <returns></returns>
		public List<Shgx004Info> GetListByPage(string textWhere, string textOrder, int pageIndex, int pageRow)
		{
            ...
            string sql = "select top " + pageRow + " * from "
                ...
                + ",* from [LES].[V_WMM_SHGX_004]  WITH(NOLOCK) " + whereText + ") T "
        ...
        {			
			...
			Database db = DatabaseFactory.CreateDatabase();
            DbCommand cmd = db.GetSqlStringCommand(string.Format(V_WMM_SHGX_004_SELECT_COUNTS, textWhere));
		...
		public bool Add(Shgx004Info info)
		{ 
			Database db = DatabaseFactory.CreateDatabase();	
			DbCommand dbCommand = db.GetSqlStringCommand(V_WMM_SHGX_004_INSERT);			
		...
		/// </summary>
		/// <param name="info">info</param>
		public int Update(Shgx004Info info)
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(V_WMM_SHGX_004_UPDATE);				
			db.AddInParameter(dbCommand, "@CustTrustNo", DbType.String, info.Custtrustno);
			db.AddInParameter(dbCommand, "@CustName", DbType.String, info.Custname);
			db.AddInParameter(dbCommand, "@Ysf", DbType.Decimal, info.Ysf);
			db.AddInParameter(dbCommand, "@Qtfy", DbType.Decimal, info.Qtfy);
		...
		private static Shgx004Info CreateShgx004Info(IDataReader rdr)
		{
			Shgx004Info info = new Shgx004Info();
