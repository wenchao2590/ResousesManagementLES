#region Declaim
//---------------------------------------------------------------------------
// Name:		WmsVmiAsnRunsheetDAL
// Function: 	Expose data in table TI_IFM_WMS_VMI_ASN_RUNSHEET from database as business object to MES system.
// Tool:		T4
...
{     
	//// <summary>
    /// WmsVmiAsnRunsheetDAL对应表[TI_IFM_WMS_VMI_ASN_RUNSHEET]
    /// </summary>
    public partial class WmsVmiAsnRunsheetDAL : BusinessObjectProvider<WmsVmiAsnRunsheetInfo>
	{
		#region Sql Statements
		private const string TI_IFM_WMS_VMI_ASN_RUNSHEET_SELECT_BY_ID =
			@"SELECT ID,
				FID,
				LOG_FID,
				ORDERCODE,
				...
				MODIFY_USER,
				COMMENTS				  
				FROM [LES].[TI_IFM_WMS_VMI_ASN_RUNSHEET] WITH(NOLOCK) WHERE [VALID_FLAG] = 1  AND ID =@ID;";
			
		private const string TI_IFM_WMS_VMI_ASN_RUNSHEET_SELECT = 
			@"SELECT ID,
				FID,
				LOG_FID,
				ORDERCODE,
				...
				MODIFY_USER,
				COMMENTS				 
				FROM [LES].[TI_IFM_WMS_VMI_ASN_RUNSHEET] WITH (NOLOCK) WHERE [VALID_FLAG] = 1 {0};";
		
		private const string TI_IFM_WMS_VMI_ASN_RUNSHEET_SELECT_COUNTS = 
			@"SELECT count(*) FROM [LES].[TI_IFM_WMS_VMI_ASN_RUNSHEET]  WITH(NOLOCK) WHERE [VALID_FLAG] = 1 {0};";

		private const string TI_IFM_WMS_VMI_ASN_RUNSHEET_INSERT =
			@"INSERT INTO [LES].[TI_IFM_WMS_VMI_ASN_RUNSHEET] (
				FID,
				LOG_FID,
				ORDERCODE,
				PLANT,
		...
		private const string TI_IFM_WMS_VMI_ASN_RUNSHEET_UPDATE =
			@"UPDATE [LES].[TI_IFM_WMS_VMI_ASN_RUNSHEET] WITH(ROWLOCK) 
				SET FID=@FID,
				LOG_FID=@LOG_FID,
				ORDERCODE=@ORDERCODE,
				PLANT=@PLANT,
		...
		private const string TI_IFM_WMS_VMI_ASN_RUNSHEET_DELETE =
			@"DELETE FROM [LES].[TI_IFM_WMS_VMI_ASN_RUNSHEET] WITH(ROWLOCK)  
				WHERE [VALID_FLAG] = 1  AND ID =@ID;";
		#endregion
		...
		/// <summary>
		/// Get WmsVmiAsnRunsheetInfo
		/// </summary>
		/// <param name="ID">WmsVmiAsnRunsheetInfo Primary key </param>
		/// <returns></returns> 
		public WmsVmiAsnRunsheetInfo GetInfo(long aId)
		{	
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TI_IFM_WMS_VMI_ASN_RUNSHEET_SELECT_BY_ID);
			db.AddInParameter(dbCommand, "@ID", DbType.Int64, aId);
			using (IDataReader dr = db.ExecuteReader(dbCommand))
			{
				if (dr.Read())
					return CreateWmsVmiAsnRunsheetInfo(dr);
			}
			return null;
		}
		/// <summary>
		...
		/// <returns>WmsVmiAsnRunsheetInfo Collection </returns>
		public List<WmsVmiAsnRunsheetInfo> GetList(string textWhere,string orderText)
		{
			...
			return GetList(string.Format(TI_IFM_WMS_VMI_ASN_RUNSHEET_SELECT, query));
		}		
		/// <summary>
		/// Get data collection
		/// </summary>
		...
		/// <returns>WmsVmiAsnRunsheetInfo Collection </returns>
		public List<WmsVmiAsnRunsheetInfo> GetList(string sql)
		{
			Database db = DatabaseFactory.CreateDatabase();
			...
			using (IDataReader dr = db.ExecuteReader(dbCommand))
			{
				while (dr.Read())
				{
					list.Add(CreateWmsVmiAsnRunsheetInfo(dr));
				}
			}
			return list;
		}		
        ...
        /// <returns></returns>
		public List<WmsVmiAsnRunsheetInfo> GetListByPage(string textWhere, string textOrder, int pageIndex, int pageRow)
		{
            ...
            string sql = "select top " + pageRow + " * from "
                ...
                + ",* from [LES].[TI_IFM_WMS_VMI_ASN_RUNSHEET]  WITH(NOLOCK) " + whereText + ") T "
        ...
        {			
			...
			Database db = DatabaseFactory.CreateDatabase();
            DbCommand cmd = db.GetSqlStringCommand(string.Format(TI_IFM_WMS_VMI_ASN_RUNSHEET_SELECT_COUNTS, textWhere));
		...
		public long Add(WmsVmiAsnRunsheetInfo info)
		{ 
			Database db = DatabaseFactory.CreateDatabase();	
			DbCommand dbCommand = db.GetSqlStringCommand(TI_IFM_WMS_VMI_ASN_RUNSHEET_INSERT);			
		...
		/// </summary>
		/// <param name="info"> info</param>
		public static string GetInsertSql(WmsVmiAsnRunsheetInfo info)
		{
			return  
			@"insert into [LES].[TI_IFM_WMS_VMI_ASN_RUNSHEET] (
				FID,
		...
		/// </summary>
		/// <param name="info">info</param>
		public int Update(WmsVmiAsnRunsheetInfo info)
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TI_IFM_WMS_VMI_ASN_RUNSHEET_UPDATE);				
		...
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TI_IFM_WMS_VMI_ASN_RUNSHEET_DELETE);
		    db.AddInParameter(dbCommand, "@ID", DbType.Int64, aId);
			return int.Parse("0" + db.ExecuteNonQuery(dbCommand));
		}/// <summary>
		/// LogciDelete
		...
		{
		    string sql = "update [LES].[TI_IFM_WMS_VMI_ASN_RUNSHEET] WITH(ROWLOCK) "
