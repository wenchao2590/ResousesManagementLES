...
//---------------------------------------------------------------------------
// Name:		VmiTransDAL
// Function: 	Expose data in table TT_WMM_VMI_TRANS from database as business object to MES system.
// Tool:		T4
...
{     
	//// <summary>
    /// VmiTransDAL对应表[TT_WMM_VMI_TRANS]
    /// </summary>
    public partial class VmiTransDAL : BusinessObjectProvider<VmiTransInfo>
	{
		#region Sql Statements
		private const string TT_WMM_VMI_TRANS_SELECT_BY_ID =
			@"SELECT ID,
				FID,
				OUTPUT_NO,
				PLANT,
				...
				SUM_PACKAGE_QTY,
				PULL_MODE				  
				FROM [LES].[TT_WMM_VMI_TRANS] WITH(NOLOCK) WHERE [VALID_FLAG] = 1  AND ID =@ID;";
			
		private const string TT_WMM_VMI_TRANS_SELECT = 
			@"SELECT ID,
				FID,
				OUTPUT_NO,
				PLANT,
				...
				SUM_PACKAGE_QTY,
				PULL_MODE				 
				FROM [LES].[TT_WMM_VMI_TRANS] WITH (NOLOCK) WHERE [VALID_FLAG] = 1 {0};";
		
		private const string TT_WMM_VMI_TRANS_SELECT_COUNTS = 
			@"SELECT count(*) FROM [LES].[TT_WMM_VMI_TRANS]  WITH(NOLOCK) WHERE [VALID_FLAG] = 1 {0};";

		private const string TT_WMM_VMI_TRANS_INSERT =
			@"INSERT INTO [LES].[TT_WMM_VMI_TRANS] (
				FID,
				OUTPUT_NO,
				PLANT,
				SUPPLIER_NUM,
		...
		private const string TT_WMM_VMI_TRANS_UPDATE =
			@"UPDATE [LES].[TT_WMM_VMI_TRANS] WITH(ROWLOCK) 
				SET FID=@FID,
				OUTPUT_NO=@OUTPUT_NO,
				PLANT=@PLANT,
		...
		private const string TT_WMM_VMI_TRANS_DELETE =
			@"DELETE FROM [LES].[TT_WMM_VMI_TRANS] WITH(ROWLOCK)  
				WHERE [VALID_FLAG] = 1  AND ID =@ID;";
		#endregion
		...
		/// Get VmiTransInfo
		/// </summary>
		/// <param name="ID">VmiTransInfo Primary key </param>
		/// <returns></returns> 
		public VmiTransInfo GetInfo(long aId)
		{	
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TT_WMM_VMI_TRANS_SELECT_BY_ID);
			db.AddInParameter(dbCommand, "@ID", DbType.Int64, aId);
			using (IDataReader dr = db.ExecuteReader(dbCommand))
			{
				if (dr.Read())
					return CreateVmiTransInfo(dr);
			}
			return null;
		}
		/// <summary>
		...
		/// <returns>VmiTransInfo Collection </returns>
		public List<VmiTransInfo> GetList(string textWhere,string orderText)
		{
			...
			return GetList(string.Format(TT_WMM_VMI_TRANS_SELECT, query));
		}		
		/// <summary>
		/// Get data collection
		/// </summary>
		...
		/// <returns>VmiTransInfo Collection </returns>
		public List<VmiTransInfo> GetList(string sql)
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(sql);
			List<VmiTransInfo> list = new List<VmiTransInfo>();
			using (IDataReader dr = db.ExecuteReader(dbCommand))
			{
				while (dr.Read())
				{
					list.Add(CreateVmiTransInfo(dr));
				}
			}
			return list;
		}		
        ...
        /// <returns></returns>
		public List<VmiTransInfo> GetListByPage(string textWhere, string textOrder, int pageIndex, int pageRow)
		{
            ...
            string sql = "select top " + pageRow + " * from "
                ...
                + ",* from [LES].[TT_WMM_VMI_TRANS]  WITH(NOLOCK) " + whereText + ") T "
        ...
        {			
			...
			Database db = DatabaseFactory.CreateDatabase();
            DbCommand cmd = db.GetSqlStringCommand(string.Format(TT_WMM_VMI_TRANS_SELECT_COUNTS, textWhere));
		...
		public long Add(VmiTransInfo info)
		{ 
			Database db = DatabaseFactory.CreateDatabase();	
			DbCommand dbCommand = db.GetSqlStringCommand(TT_WMM_VMI_TRANS_INSERT);			
		...
		/// </summary>
		/// <param name="info"> info</param>
		public static string GetInsertSql(VmiTransInfo info)
		{
			return  
			@"insert into [LES].[TT_WMM_VMI_TRANS] (
				FID,
				...
				(info.TransType == null ? "NULL" : "" + info.TransType.GetValueOrDefault() + "") + ","+
		...
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TT_WMM_VMI_TRANS_DELETE);
		    db.AddInParameter(dbCommand, "@ID", DbType.Int64, aId);
			return int.Parse("0" + db.ExecuteNonQuery(dbCommand));
		}/// <summary>
		/// LogciDelete
		...
		{
		    string sql = "update [LES].[TT_WMM_VMI_TRANS] WITH(ROWLOCK) "
		...
		private static VmiTransInfo CreateVmiTransInfo(IDataReader rdr)
		{
			...
			info.TransType = DBConvert.GetInt32Nullable(rdr, rdr.GetOrdinal("TRANS_TYPE"));			
