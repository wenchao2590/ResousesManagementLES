#region Declaim
//---------------------------------------------------------------------------
// Name:		VmiOutputDetailDAL
// Function: 	Expose data in table TT_WMM_VMI_OUTPUT_DETAIL from database as business object to MES system.
// Tool:		T4
...
{     
	//// <summary>
    /// VmiOutputDetailDAL对应表[TT_WMM_VMI_OUTPUT_DETAIL]
    /// </summary>
    public partial class VmiOutputDetailDAL : BusinessObjectProvider<VmiOutputDetailInfo>
	{
		#region Sql Statements
		private const string TT_WMM_VMI_OUTPUT_DETAIL_SELECT_BY_ID =
			@"SELECT ID,
				FID,
				OUTPUT_FID,
				PLANT,
				...
				SUM_VOLUME,
				FROZEN_STOCK_FLAG				  
				FROM [LES].[TT_WMM_VMI_OUTPUT_DETAIL] WITH(NOLOCK) WHERE [VALID_FLAG] = 1  AND ID =@ID;";
			
		private const string TT_WMM_VMI_OUTPUT_DETAIL_SELECT = 
			@"SELECT ID,
				FID,
				OUTPUT_FID,
				PLANT,
				...
				SUM_VOLUME,
				FROZEN_STOCK_FLAG				 
				FROM [LES].[TT_WMM_VMI_OUTPUT_DETAIL] WITH (NOLOCK) WHERE [VALID_FLAG] = 1 {0};";
		
		private const string TT_WMM_VMI_OUTPUT_DETAIL_SELECT_COUNTS = 
			@"SELECT count(*) FROM [LES].[TT_WMM_VMI_OUTPUT_DETAIL]  WITH(NOLOCK) WHERE [VALID_FLAG] = 1 {0};";

		private const string TT_WMM_VMI_OUTPUT_DETAIL_INSERT =
			@"INSERT INTO [LES].[TT_WMM_VMI_OUTPUT_DETAIL] (
				FID,
				OUTPUT_FID,
				PLANT,
				SUPPLIER_NUM,
		...
		private const string TT_WMM_VMI_OUTPUT_DETAIL_UPDATE =
			@"UPDATE [LES].[TT_WMM_VMI_OUTPUT_DETAIL] WITH(ROWLOCK) 
				SET FID=@FID,
		...
		private const string TT_WMM_VMI_OUTPUT_DETAIL_DELETE =
			@"DELETE FROM [LES].[TT_WMM_VMI_OUTPUT_DETAIL] WITH(ROWLOCK)  
				WHERE [VALID_FLAG] = 1  AND ID =@ID;";
		#endregion
		...
		/// <summary>
		/// Get VmiOutputDetailInfo
		/// </summary>
		/// <param name="ID">VmiOutputDetailInfo Primary key </param>
		/// <returns></returns> 
		public VmiOutputDetailInfo GetInfo(long aId)
		{	
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TT_WMM_VMI_OUTPUT_DETAIL_SELECT_BY_ID);
			db.AddInParameter(dbCommand, "@ID", DbType.Int64, aId);
			using (IDataReader dr = db.ExecuteReader(dbCommand))
			{
				if (dr.Read())
					return CreateVmiOutputDetailInfo(dr);
			}
			return null;
		}
		/// <summary>
		...
		/// <returns>VmiOutputDetailInfo Collection </returns>
		public List<VmiOutputDetailInfo> GetList(string textWhere,string orderText)
		{
			...
			return GetList(string.Format(TT_WMM_VMI_OUTPUT_DETAIL_SELECT, query));
		}		
		/// <summary>
		/// Get data collection
		/// </summary>
		...
		/// <returns>VmiOutputDetailInfo Collection </returns>
		public List<VmiOutputDetailInfo> GetList(string sql)
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(sql);
			List<VmiOutputDetailInfo> list = new List<VmiOutputDetailInfo>();
			using (IDataReader dr = db.ExecuteReader(dbCommand))
			{
				while (dr.Read())
				{
					list.Add(CreateVmiOutputDetailInfo(dr));
				}
			}
			return list;
		}		
        ...
        /// <returns></returns>
		public List<VmiOutputDetailInfo> GetListByPage(string textWhere, string textOrder, int pageIndex, int pageRow)
		{
            ...
            string sql = "select top " + pageRow + " * from "
                ...
                + ",* from [LES].[TT_WMM_VMI_OUTPUT_DETAIL]  WITH(NOLOCK) " + whereText + ") T "
        ...
        {			
			...
			Database db = DatabaseFactory.CreateDatabase();
            DbCommand cmd = db.GetSqlStringCommand(string.Format(TT_WMM_VMI_OUTPUT_DETAIL_SELECT_COUNTS, textWhere));
		...
		public long Add(VmiOutputDetailInfo info)
		{ 
			Database db = DatabaseFactory.CreateDatabase();	
			DbCommand dbCommand = db.GetSqlStringCommand(TT_WMM_VMI_OUTPUT_DETAIL_INSERT);			
		...
		/// </summary>
		/// <param name="info"> info</param>
		public static string GetInsertSql(VmiOutputDetailInfo info)
		{
			return  
			@"insert into [LES].[TT_WMM_VMI_OUTPUT_DETAIL] (
				FID,
		...
		/// </summary>
		/// <param name="info">info</param>
		public int Update(VmiOutputDetailInfo info)
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TT_WMM_VMI_OUTPUT_DETAIL_UPDATE);				
		...
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TT_WMM_VMI_OUTPUT_DETAIL_DELETE);
		    db.AddInParameter(dbCommand, "@ID", DbType.Int64, aId);
			return int.Parse("0" + db.ExecuteNonQuery(dbCommand));
		}/// <summary>
		/// LogciDelete
		...
		{
		    string sql = "update [LES].[TT_WMM_VMI_OUTPUT_DETAIL] WITH(ROWLOCK) "
		...
		private static VmiOutputDetailInfo CreateVmiOutputDetailInfo(IDataReader rdr)
		{
