...
//---------------------------------------------------------------------------
// Name:		Shgx001DAL
// Function: 	Expose data in table V_WMM_SHGX_001 from database as business object to MES system.
// Tool:		T4
...
{     
	//// <summary>
    /// Shgx001DAL对应表[V_WMM_SHGX_001]
    /// </summary>
    public partial class Shgx001DAL : BusinessObjectProvider<Shgx001Info>
	{
		#region Sql Statements
		private const string V_WMM_SHGX_001_SELECT_BY_ID =
			"";
			
		private const string V_WMM_SHGX_001_SELECT = 
			@"SELECT CustTrustNo,
				CustName,
				Ysf,
				Qtfy,
				...
				TargetZone,
				Jcf				 
				FROM [LES].[V_WMM_SHGX_001] WITH (NOLOCK) WHERE 1=1 {0};";
		
		private const string V_WMM_SHGX_001_SELECT_COUNTS = 
			@"SELECT count(*) FROM [LES].[V_WMM_SHGX_001]  WITH(NOLOCK) WHERE 1=1 {0};";

		private const string V_WMM_SHGX_001_INSERT =
			@"INSERT INTO [LES].[V_WMM_SHGX_001] (
				CustTrustNo,
				CustName,
				Ysf,
				Qtfy,
		...
		private const string V_WMM_SHGX_001_UPDATE =
			"";

		private const string V_WMM_SHGX_001_DELETE =
			"";
		#endregion
		...
		/// <returns>Shgx001Info Collection </returns>
		public List<Shgx001Info> GetList(string textWhere,string orderText)
		{
			...
			return GetList(string.Format(V_WMM_SHGX_001_SELECT, query));
		}		
		/// <summary>
		/// Get data collection
		/// </summary>
		...
		/// <returns>Shgx001Info Collection </returns>
		public List<Shgx001Info> GetList(string sql)
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(sql);
			List<Shgx001Info> list = new List<Shgx001Info>();
			using (IDataReader dr = db.ExecuteReader(dbCommand))
			{
				while (dr.Read())
				{
					list.Add(CreateShgx001Info(dr));
				}
			}
			return list;
		}		
        ...
        /// <returns></returns>
		public List<Shgx001Info> GetListByPage(string textWhere, string textOrder, int pageIndex, int pageRow)
		{
            ...
            string sql = "select top " + pageRow + " * from "
                ...
                + ",* from [LES].[V_WMM_SHGX_001]  WITH(NOLOCK) " + whereText + ") T "
        ...
        {			
			...
			Database db = DatabaseFactory.CreateDatabase();
            DbCommand cmd = db.GetSqlStringCommand(string.Format(V_WMM_SHGX_001_SELECT_COUNTS, textWhere));
		...
		public bool Add(Shgx001Info info)
		{ 
			Database db = DatabaseFactory.CreateDatabase();	
			DbCommand dbCommand = db.GetSqlStringCommand(V_WMM_SHGX_001_INSERT);			
		...
		/// </summary>
		/// <param name="info">info</param>
		public int Update(Shgx001Info info)
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(V_WMM_SHGX_001_UPDATE);				
			db.AddInParameter(dbCommand, "@CustTrustNo", DbType.String, info.Custtrustno);
			db.AddInParameter(dbCommand, "@CustName", DbType.String, info.Custname);
			db.AddInParameter(dbCommand, "@Ysf", DbType.Decimal, info.Ysf);
			db.AddInParameter(dbCommand, "@Qtfy", DbType.Decimal, info.Qtfy);
		...
		private static Shgx001Info CreateShgx001Info(IDataReader rdr)
		{
			Shgx001Info info = new Shgx001Info();
