#region Declaim
//---------------------------------------------------------------------------
// Name:		SpsPullOrderDetailDAL
// Function: 	Expose data in table TT_MPM_SPS_PULL_ORDER_DETAIL from database as business object to MES system.
// Tool:		T4
...
{     
	//// <summary>
    /// SpsPullOrderDetailDAL对应表[TT_MPM_SPS_PULL_ORDER_DETAIL]
    /// </summary>
    public partial class SpsPullOrderDetailDAL : BusinessObjectProvider<SpsPullOrderDetailInfo>
	{
		#region Sql Statements
		private const string TT_MPM_SPS_PULL_ORDER_DETAIL_SELECT_BY_ID =
			@"SELECT ID,
				FID,
				ORDER_FID,
				ORDER_CODE,
				...
				MODIFY_DATE,
				MODIFY_USER				  
				FROM [LES].[TT_MPM_SPS_PULL_ORDER_DETAIL] WITH(NOLOCK) WHERE [VALID_FLAG] = 1  AND ID =@ID;";
			
		private const string TT_MPM_SPS_PULL_ORDER_DETAIL_SELECT = 
			@"SELECT ID,
				FID,
				ORDER_FID,
				ORDER_CODE,
				...
				MODIFY_DATE,
				MODIFY_USER				 
				FROM [LES].[TT_MPM_SPS_PULL_ORDER_DETAIL] WITH (NOLOCK) WHERE [VALID_FLAG] = 1 {0};";
		
		private const string TT_MPM_SPS_PULL_ORDER_DETAIL_SELECT_COUNTS = 
			@"SELECT count(*) FROM [LES].[TT_MPM_SPS_PULL_ORDER_DETAIL]  WITH(NOLOCK) WHERE [VALID_FLAG] = 1 {0};";

		private const string TT_MPM_SPS_PULL_ORDER_DETAIL_INSERT =
			@"INSERT INTO [LES].[TT_MPM_SPS_PULL_ORDER_DETAIL] (
				FID,
				ORDER_FID,
				ORDER_CODE,
				ROW_NO,
		...
		private const string TT_MPM_SPS_PULL_ORDER_DETAIL_UPDATE =
			@"UPDATE [LES].[TT_MPM_SPS_PULL_ORDER_DETAIL] WITH(ROWLOCK) 
				SET FID=@FID,
				ORDER_FID=@ORDER_FID,
		...
		private const string TT_MPM_SPS_PULL_ORDER_DETAIL_DELETE =
			@"DELETE FROM [LES].[TT_MPM_SPS_PULL_ORDER_DETAIL] WITH(ROWLOCK)  
				WHERE [VALID_FLAG] = 1  AND ID =@ID;";
		#endregion
		...
		/// <summary>
		/// Get SpsPullOrderDetailInfo
		/// </summary>
		/// <param name="ID">SpsPullOrderDetailInfo Primary key </param>
		/// <returns></returns> 
		public SpsPullOrderDetailInfo GetInfo(long aId)
		{	
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TT_MPM_SPS_PULL_ORDER_DETAIL_SELECT_BY_ID);
			db.AddInParameter(dbCommand, "@ID", DbType.Int64, aId);
			using (IDataReader dr = db.ExecuteReader(dbCommand))
			{
				if (dr.Read())
					return CreateSpsPullOrderDetailInfo(dr);
			}
			return null;
		}
		/// <summary>
		...
		/// <returns>SpsPullOrderDetailInfo Collection </returns>
		public List<SpsPullOrderDetailInfo> GetList(string textWhere,string orderText)
		{
			...
			return GetList(string.Format(TT_MPM_SPS_PULL_ORDER_DETAIL_SELECT, query));
		}		
		/// <summary>
		/// Get data collection
		/// </summary>
		...
		/// <returns>SpsPullOrderDetailInfo Collection </returns>
		public List<SpsPullOrderDetailInfo> GetList(string sql)
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(sql);
			List<SpsPullOrderDetailInfo> list = new List<SpsPullOrderDetailInfo>();
			using (IDataReader dr = db.ExecuteReader(dbCommand))
			{
				while (dr.Read())
				{
					list.Add(CreateSpsPullOrderDetailInfo(dr));
				}
			}
			return list;
		}		
        ...
        /// <returns></returns>
		public List<SpsPullOrderDetailInfo> GetListByPage(string textWhere, string textOrder, int pageIndex, int pageRow)
		{
            ...
            string sql = "select top " + pageRow + " * from "
                ...
                + ",* from [LES].[TT_MPM_SPS_PULL_ORDER_DETAIL]  WITH(NOLOCK) " + whereText + ") T "
        ...
        {			
			...
			Database db = DatabaseFactory.CreateDatabase();
            DbCommand cmd = db.GetSqlStringCommand(string.Format(TT_MPM_SPS_PULL_ORDER_DETAIL_SELECT_COUNTS, textWhere));
		...
		public long Add(SpsPullOrderDetailInfo info)
		{ 
			Database db = DatabaseFactory.CreateDatabase();	
			DbCommand dbCommand = db.GetSqlStringCommand(TT_MPM_SPS_PULL_ORDER_DETAIL_INSERT);			
			...
			db.AddInParameter(dbCommand, "@LIGHT_ADDRESS", DbType.String, info.LightAddress);
		...
		/// </summary>
		/// <param name="info"> info</param>
		public static string GetInsertSql(SpsPullOrderDetailInfo info)
		{
			return  
			@"insert into [LES].[TT_MPM_SPS_PULL_ORDER_DETAIL] (
				FID,
				...
				(string.IsNullOrEmpty(info.LightAddress) ? "NULL" : "N'" + info.LightAddress + "'") + ","+
		...
		/// </summary>
		/// <param name="info">info</param>
		public int Update(SpsPullOrderDetailInfo info)
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TT_MPM_SPS_PULL_ORDER_DETAIL_UPDATE);				
		...
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TT_MPM_SPS_PULL_ORDER_DETAIL_DELETE);
		    db.AddInParameter(dbCommand, "@ID", DbType.Int64, aId);
			return int.Parse("0" + db.ExecuteNonQuery(dbCommand));
		}/// <summary>
		/// LogciDelete
		...
		{
		    string sql = "update [LES].[TT_MPM_SPS_PULL_ORDER_DETAIL] WITH(ROWLOCK) "
		...
		private static SpsPullOrderDetailInfo CreateSpsPullOrderDetailInfo(IDataReader rdr)
		{
			...
			info.LightAddress = DBConvert.GetString(rdr, rdr.GetOrdinal("LIGHT_ADDRESS"));			
