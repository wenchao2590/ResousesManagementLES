#region Declaim
//---------------------------------------------------------------------------
// Name:		SrmDeliveryNoteDAL
// Function: 	Expose data in table TI_IFM_SRM_DELIVERY_NOTE from database as business object to MES system.
// Tool:		T4
...
{     
	//// <summary>
    /// SrmDeliveryNoteDAL对应表[TI_IFM_SRM_DELIVERY_NOTE]
    /// </summary>
    public partial class SrmDeliveryNoteDAL : BusinessObjectProvider<SrmDeliveryNoteInfo>
	{
		#region Sql Statements
		private const string TI_IFM_SRM_DELIVERY_NOTE_SELECT_BY_ID =
			@"SELECT ID,
				FID,
				LOG_FID,
				ORDERCODE,
				...
				MODIFY_DATE,
				MODIFY_USER				  
				FROM [LES].[TI_IFM_SRM_DELIVERY_NOTE] WITH(NOLOCK) WHERE [VALID_FLAG] = 1  AND ID =@ID;";
			
		private const string TI_IFM_SRM_DELIVERY_NOTE_SELECT = 
			@"SELECT ID,
				FID,
				LOG_FID,
				ORDERCODE,
				...
				MODIFY_DATE,
				MODIFY_USER				 
				FROM [LES].[TI_IFM_SRM_DELIVERY_NOTE] WITH (NOLOCK) WHERE [VALID_FLAG] = 1 {0};";
		
		private const string TI_IFM_SRM_DELIVERY_NOTE_SELECT_COUNTS = 
			@"SELECT count(*) FROM [LES].[TI_IFM_SRM_DELIVERY_NOTE]  WITH(NOLOCK) WHERE [VALID_FLAG] = 1 {0};";

		private const string TI_IFM_SRM_DELIVERY_NOTE_INSERT =
			@"INSERT INTO [LES].[TI_IFM_SRM_DELIVERY_NOTE] (
				FID,
				LOG_FID,
				ORDERCODE,
				PLANT,
		...
		private const string TI_IFM_SRM_DELIVERY_NOTE_UPDATE =
			@"UPDATE [LES].[TI_IFM_SRM_DELIVERY_NOTE] WITH(ROWLOCK) 
				SET FID=@FID,
				LOG_FID=@LOG_FID,
				ORDERCODE=@ORDERCODE,
				PLANT=@PLANT,
		...
		private const string TI_IFM_SRM_DELIVERY_NOTE_DELETE =
			@"DELETE FROM [LES].[TI_IFM_SRM_DELIVERY_NOTE] WITH(ROWLOCK)  
				WHERE [VALID_FLAG] = 1  AND ID =@ID;";
		#endregion
		...
		/// <summary>
		/// Get SrmDeliveryNoteInfo
		/// </summary>
		/// <param name="ID">SrmDeliveryNoteInfo Primary key </param>
		/// <returns></returns> 
		public SrmDeliveryNoteInfo GetInfo(long aId)
		{	
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TI_IFM_SRM_DELIVERY_NOTE_SELECT_BY_ID);
			db.AddInParameter(dbCommand, "@ID", DbType.Int64, aId);
			using (IDataReader dr = db.ExecuteReader(dbCommand))
			{
				if (dr.Read())
					return CreateSrmDeliveryNoteInfo(dr);
			}
			return null;
		}
		/// <summary>
		...
		/// <returns>SrmDeliveryNoteInfo Collection </returns>
		public List<SrmDeliveryNoteInfo> GetList(string textWhere,string orderText)
		{
			...
			return GetList(string.Format(TI_IFM_SRM_DELIVERY_NOTE_SELECT, query));
		}		
		/// <summary>
		/// Get data collection
		/// </summary>
		...
		/// <returns>SrmDeliveryNoteInfo Collection </returns>
		public List<SrmDeliveryNoteInfo> GetList(string sql)
		{
			Database db = DatabaseFactory.CreateDatabase();
			...
			using (IDataReader dr = db.ExecuteReader(dbCommand))
			{
				while (dr.Read())
				{
					list.Add(CreateSrmDeliveryNoteInfo(dr));
				}
			}
			return list;
		}		
        ...
        /// <returns></returns>
		public List<SrmDeliveryNoteInfo> GetListByPage(string textWhere, string textOrder, int pageIndex, int pageRow)
		{
            ...
            string sql = "select top " + pageRow + " * from "
                ...
                + ",* from [LES].[TI_IFM_SRM_DELIVERY_NOTE]  WITH(NOLOCK) " + whereText + ") T "
        ...
        {			
			...
			Database db = DatabaseFactory.CreateDatabase();
            DbCommand cmd = db.GetSqlStringCommand(string.Format(TI_IFM_SRM_DELIVERY_NOTE_SELECT_COUNTS, textWhere));
		...
		public long Add(SrmDeliveryNoteInfo info)
		{ 
			Database db = DatabaseFactory.CreateDatabase();	
			DbCommand dbCommand = db.GetSqlStringCommand(TI_IFM_SRM_DELIVERY_NOTE_INSERT);			
		...
		/// </summary>
		/// <param name="info"> info</param>
		public static string GetInsertSql(SrmDeliveryNoteInfo info)
		{
			return  
			@"insert into [LES].[TI_IFM_SRM_DELIVERY_NOTE] (
				FID,
		...
		/// </summary>
		/// <param name="info">info</param>
		public int Update(SrmDeliveryNoteInfo info)
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TI_IFM_SRM_DELIVERY_NOTE_UPDATE);				
		...
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TI_IFM_SRM_DELIVERY_NOTE_DELETE);
		    db.AddInParameter(dbCommand, "@ID", DbType.Int64, aId);
			return int.Parse("0" + db.ExecuteNonQuery(dbCommand));
		}/// <summary>
		/// LogciDelete
		...
		{
		    string sql = "update [LES].[TI_IFM_SRM_DELIVERY_NOTE] WITH(ROWLOCK) "
		...
		private static SrmDeliveryNoteInfo CreateSrmDeliveryNoteInfo(IDataReader rdr)
		{
