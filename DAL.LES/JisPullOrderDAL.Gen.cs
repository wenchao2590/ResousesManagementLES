#region Declaim
//---------------------------------------------------------------------------
// Name:		JisPullOrderDAL
// Function: 	Expose data in table TT_MPM_JIS_PULL_ORDER from database as business object to MES system.
// Tool:		T4
...
{     
	//// <summary>
    /// JisPullOrderDAL对应表[TT_MPM_JIS_PULL_ORDER]
    /// </summary>
    public partial class JisPullOrderDAL : BusinessObjectProvider<JisPullOrderInfo>
	{
		#region Sql Statements
		private const string TT_MPM_JIS_PULL_ORDER_SELECT_BY_ID =
			@"SELECT ID,
				FID,
				ORDER_CODE,
				PART_BOX_CODE,
				...
				MODIFY_DATE,
				MODIFY_USER				  
				FROM [LES].[TT_MPM_JIS_PULL_ORDER] WITH(NOLOCK) WHERE [VALID_FLAG] = 1  AND ID =@ID;";
			
		private const string TT_MPM_JIS_PULL_ORDER_SELECT = 
			@"SELECT ID,
				FID,
				ORDER_CODE,
				PART_BOX_CODE,
				...
				MODIFY_DATE,
				MODIFY_USER				 
				FROM [LES].[TT_MPM_JIS_PULL_ORDER] WITH (NOLOCK) WHERE [VALID_FLAG] = 1 {0};";
		
		private const string TT_MPM_JIS_PULL_ORDER_SELECT_COUNTS = 
			@"SELECT count(*) FROM [LES].[TT_MPM_JIS_PULL_ORDER]  WITH(NOLOCK) WHERE [VALID_FLAG] = 1 {0};";

		private const string TT_MPM_JIS_PULL_ORDER_INSERT =
			@"INSERT INTO [LES].[TT_MPM_JIS_PULL_ORDER] (
				FID,
				ORDER_CODE,
				PART_BOX_CODE,
				PART_BOX_NAME,
		...
		private const string TT_MPM_JIS_PULL_ORDER_UPDATE =
			@"UPDATE [LES].[TT_MPM_JIS_PULL_ORDER] WITH(ROWLOCK) 
				SET FID=@FID,
				...
				START_VEHICHE_NO=@START_VEHICHE_NO,
				END_VEHICHE_NO=@END_VEHICHE_NO,
				DAY_SEQ_NO=@DAY_SEQ_NO,
				KEEPER=@KEEPER,
		...
		private const string TT_MPM_JIS_PULL_ORDER_DELETE =
			@"DELETE FROM [LES].[TT_MPM_JIS_PULL_ORDER] WITH(ROWLOCK)  
				WHERE [VALID_FLAG] = 1  AND ID =@ID;";
		#endregion
		...
		/// <summary>
		/// Get JisPullOrderInfo
		/// </summary>
		/// <param name="ID">JisPullOrderInfo Primary key </param>
		/// <returns></returns> 
		public JisPullOrderInfo GetInfo(long aId)
		{	
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TT_MPM_JIS_PULL_ORDER_SELECT_BY_ID);
			db.AddInParameter(dbCommand, "@ID", DbType.Int64, aId);
			using (IDataReader dr = db.ExecuteReader(dbCommand))
			{
				if (dr.Read())
					return CreateJisPullOrderInfo(dr);
			}
			return null;
		}
		/// <summary>
		...
		/// <returns>JisPullOrderInfo Collection </returns>
		public List<JisPullOrderInfo> GetList(string textWhere,string orderText)
		{
			...
			return GetList(string.Format(TT_MPM_JIS_PULL_ORDER_SELECT, query));
		}		
		/// <summary>
		/// Get data collection
		/// </summary>
		...
		/// <returns>JisPullOrderInfo Collection </returns>
		public List<JisPullOrderInfo> GetList(string sql)
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(sql);
			List<JisPullOrderInfo> list = new List<JisPullOrderInfo>();
			using (IDataReader dr = db.ExecuteReader(dbCommand))
			{
				while (dr.Read())
				{
					list.Add(CreateJisPullOrderInfo(dr));
				}
			}
			return list;
		}		
        ...
        /// <returns></returns>
		public List<JisPullOrderInfo> GetListByPage(string textWhere, string textOrder, int pageIndex, int pageRow)
		{
            ...
            string sql = "select top " + pageRow + " * from "
                ...
                + ",* from [LES].[TT_MPM_JIS_PULL_ORDER]  WITH(NOLOCK) " + whereText + ") T "
        ...
        {			
			...
			Database db = DatabaseFactory.CreateDatabase();
            DbCommand cmd = db.GetSqlStringCommand(string.Format(TT_MPM_JIS_PULL_ORDER_SELECT_COUNTS, textWhere));
		...
		public long Add(JisPullOrderInfo info)
		{ 
			Database db = DatabaseFactory.CreateDatabase();	
			DbCommand dbCommand = db.GetSqlStringCommand(TT_MPM_JIS_PULL_ORDER_INSERT);			
			...
			db.AddInParameter(dbCommand, "@START_VEHICHE_NO", DbType.String, info.StartVehicheNo);
			db.AddInParameter(dbCommand, "@END_VEHICHE_NO", DbType.String, info.EndVehicheNo);
			db.AddInParameter(dbCommand, "@DAY_SEQ_NO", DbType.Int32, info.DaySeqNo);
		...
		/// </summary>
		/// <param name="info"> info</param>
		public static string GetInsertSql(JisPullOrderInfo info)
		{
			return  
			@"insert into [LES].[TT_MPM_JIS_PULL_ORDER] (
				FID,
				...
				(string.IsNullOrEmpty(info.StartVehicheNo) ? "NULL" : "N'" + info.StartVehicheNo + "'") + ","+
				(string.IsNullOrEmpty(info.EndVehicheNo) ? "NULL" : "N'" + info.EndVehicheNo + "'") + ","+
				(info.DaySeqNo == null ? "NULL" : "" + info.DaySeqNo.GetValueOrDefault() + "") + ","+
		...
		/// </summary>
		/// <param name="info">info</param>
		public int Update(JisPullOrderInfo info)
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TT_MPM_JIS_PULL_ORDER_UPDATE);				
		...
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TT_MPM_JIS_PULL_ORDER_DELETE);
		    db.AddInParameter(dbCommand, "@ID", DbType.Int64, aId);
			return int.Parse("0" + db.ExecuteNonQuery(dbCommand));
		}/// <summary>
		/// LogciDelete
		...
		{
		    string sql = "update [LES].[TT_MPM_JIS_PULL_ORDER] WITH(ROWLOCK) "
		...
		private static JisPullOrderInfo CreateJisPullOrderInfo(IDataReader rdr)
		{
			JisPullOrderInfo info = new JisPullOrderInfo();
			...
			info.StartVehicheNo = DBConvert.GetString(rdr, rdr.GetOrdinal("START_VEHICHE_NO"));			
			info.EndVehicheNo = DBConvert.GetString(rdr, rdr.GetOrdinal("END_VEHICHE_NO"));			
			info.DaySeqNo = DBConvert.GetInt32Nullable(rdr, rdr.GetOrdinal("DAY_SEQ_NO"));			
