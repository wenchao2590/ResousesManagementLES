...
//---------------------------------------------------------------------------
// Name:		Shgx002DAL
// Function: 	Expose data in table V_WMM_SHGX_002 from database as business object to MES system.
// Tool:		T4
...
{     
	//// <summary>
    /// Shgx002DAL对应表[V_WMM_SHGX_002]
    /// </summary>
    public partial class Shgx002DAL : BusinessObjectProvider<Shgx002Info>
	{
		#region Sql Statements
		private const string V_WMM_SHGX_002_SELECT_BY_ID =
			"";
			
		private const string V_WMM_SHGX_002_SELECT = 
			@"SELECT CustTrustNo,
				CustName,
				Ysf,
				Qtfy,
				...
				TargetZone,
				Jcf				 
				FROM [LES].[V_WMM_SHGX_002] WITH (NOLOCK) WHERE 1=1 {0};";
		
		private const string V_WMM_SHGX_002_SELECT_COUNTS = 
			@"SELECT count(*) FROM [LES].[V_WMM_SHGX_002]  WITH(NOLOCK) WHERE 1=1 {0};";

		private const string V_WMM_SHGX_002_INSERT =
			@"INSERT INTO [LES].[V_WMM_SHGX_002] (
				CustTrustNo,
				CustName,
				Ysf,
				Qtfy,
		...
		private const string V_WMM_SHGX_002_UPDATE =
			"";

		private const string V_WMM_SHGX_002_DELETE =
			"";
		#endregion
		...
		/// <returns>Shgx002Info Collection </returns>
		public List<Shgx002Info> GetList(string textWhere,string orderText)
		{
			...
			return GetList(string.Format(V_WMM_SHGX_002_SELECT, query));
		}		
		/// <summary>
		/// Get data collection
		/// </summary>
		...
		/// <returns>Shgx002Info Collection </returns>
		public List<Shgx002Info> GetList(string sql)
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(sql);
			List<Shgx002Info> list = new List<Shgx002Info>();
			using (IDataReader dr = db.ExecuteReader(dbCommand))
			{
				while (dr.Read())
				{
					list.Add(CreateShgx002Info(dr));
				}
			}
			return list;
		}		
        ...
        /// <returns></returns>
		public List<Shgx002Info> GetListByPage(string textWhere, string textOrder, int pageIndex, int pageRow)
		{
            ...
            string sql = "select top " + pageRow + " * from "
                ...
                + ",* from [LES].[V_WMM_SHGX_002]  WITH(NOLOCK) " + whereText + ") T "
        ...
        {			
			...
			Database db = DatabaseFactory.CreateDatabase();
            DbCommand cmd = db.GetSqlStringCommand(string.Format(V_WMM_SHGX_002_SELECT_COUNTS, textWhere));
		...
		public bool Add(Shgx002Info info)
		{ 
			Database db = DatabaseFactory.CreateDatabase();	
			DbCommand dbCommand = db.GetSqlStringCommand(V_WMM_SHGX_002_INSERT);			
		...
		/// </summary>
		/// <param name="info">info</param>
		public int Update(Shgx002Info info)
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(V_WMM_SHGX_002_UPDATE);				
			db.AddInParameter(dbCommand, "@CustTrustNo", DbType.String, info.Custtrustno);
			db.AddInParameter(dbCommand, "@CustName", DbType.String, info.Custname);
			db.AddInParameter(dbCommand, "@Ysf", DbType.Decimal, info.Ysf);
			db.AddInParameter(dbCommand, "@Qtfy", DbType.Decimal, info.Qtfy);
		...
		private static Shgx002Info CreateShgx002Info(IDataReader rdr)
		{
