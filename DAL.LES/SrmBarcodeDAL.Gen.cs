...
//---------------------------------------------------------------------------
// Name:		SrmBarcodeDAL
// Function: 	Expose data in table TI_IFM_SRM_BARCODE from database as business object to MES system.
// Tool:		T4
...
{     
	//// <summary>
    /// SrmBarcodeDAL对应表[TI_IFM_SRM_BARCODE]
    /// </summary>
    public partial class SrmBarcodeDAL : BusinessObjectProvider<SrmBarcodeInfo>
	{
		#region Sql Statements
		private const string TI_IFM_SRM_BARCODE_SELECT_BY_ID =
			@"SELECT ID,
				FID,
				LOG_FID,
				PACKAGE_BARCODE,
				...
				MODIFY_USER,
				COMMENTS				  
				FROM [LES].[TI_IFM_SRM_BARCODE] WITH(NOLOCK) WHERE [VALID_FLAG] = 1  AND ID =@ID;";
			
		private const string TI_IFM_SRM_BARCODE_SELECT = 
			@"SELECT ID,
				FID,
				LOG_FID,
				PACKAGE_BARCODE,
				...
				MODIFY_USER,
				COMMENTS				 
				FROM [LES].[TI_IFM_SRM_BARCODE] WITH (NOLOCK) WHERE [VALID_FLAG] = 1 {0};";
		
		private const string TI_IFM_SRM_BARCODE_SELECT_COUNTS = 
			@"SELECT count(*) FROM [LES].[TI_IFM_SRM_BARCODE]  WITH(NOLOCK) WHERE [VALID_FLAG] = 1 {0};";

		private const string TI_IFM_SRM_BARCODE_INSERT =
			@"INSERT INTO [LES].[TI_IFM_SRM_BARCODE] (
				FID,
				LOG_FID,
				PACKAGE_BARCODE,
				SOURCE_ORDER_CODE,
		...
		private const string TI_IFM_SRM_BARCODE_UPDATE =
			@"UPDATE [LES].[TI_IFM_SRM_BARCODE] WITH(ROWLOCK) 
				SET FID=@FID,
				LOG_FID=@LOG_FID,
		...
		private const string TI_IFM_SRM_BARCODE_DELETE =
			@"DELETE FROM [LES].[TI_IFM_SRM_BARCODE] WITH(ROWLOCK)  
				WHERE [VALID_FLAG] = 1  AND ID =@ID;";
		#endregion
		...
		/// Get SrmBarcodeInfo
		/// </summary>
		/// <param name="ID">SrmBarcodeInfo Primary key </param>
		/// <returns></returns> 
		public SrmBarcodeInfo GetInfo(long aId)
		{	
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TI_IFM_SRM_BARCODE_SELECT_BY_ID);
			db.AddInParameter(dbCommand, "@ID", DbType.Int64, aId);
			using (IDataReader dr = db.ExecuteReader(dbCommand))
			{
				if (dr.Read())
					return CreateSrmBarcodeInfo(dr);
			}
			return null;
		}
		/// <summary>
		...
		public List<SrmBarcodeInfo> GetList(string textWhere,string orderText)
		{
			...
			return GetList(string.Format(TI_IFM_SRM_BARCODE_SELECT, query));
		}		
		/// <summary>
		/// Get data collection
		/// </summary>
		...
		/// <returns>SrmBarcodeInfo Collection </returns>
		public List<SrmBarcodeInfo> GetList(string sql)
		{
			Database db = DatabaseFactory.CreateDatabase();
			...
			using (IDataReader dr = db.ExecuteReader(dbCommand))
			{
				while (dr.Read())
				{
					list.Add(CreateSrmBarcodeInfo(dr));
				}
			}
			return list;
		}		
        ...
        /// <returns></returns>
		public List<SrmBarcodeInfo> GetListByPage(string textWhere, string textOrder, int pageIndex, int pageRow)
		{
            ...
            string sql = "select top " + pageRow + " * from "
                ...
                + ",* from [LES].[TI_IFM_SRM_BARCODE]  WITH(NOLOCK) " + whereText + ") T "
        ...
        {			
			...
			Database db = DatabaseFactory.CreateDatabase();
            DbCommand cmd = db.GetSqlStringCommand(string.Format(TI_IFM_SRM_BARCODE_SELECT_COUNTS, textWhere));
		...
		public long Add(SrmBarcodeInfo info)
		{ 
			Database db = DatabaseFactory.CreateDatabase();	
			DbCommand dbCommand = db.GetSqlStringCommand(TI_IFM_SRM_BARCODE_INSERT);			
		...
		/// </summary>
		/// <param name="info"> info</param>
		public static string GetInsertSql(SrmBarcodeInfo info)
		{
			return  
			@"insert into [LES].[TI_IFM_SRM_BARCODE] (
				FID,
		...
		/// </summary>
		/// <param name="info">info</param>
		public int Update(SrmBarcodeInfo info)
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TI_IFM_SRM_BARCODE_UPDATE);				
		...
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(TI_IFM_SRM_BARCODE_DELETE);
		    db.AddInParameter(dbCommand, "@ID", DbType.Int64, aId);
			return int.Parse("0" + db.ExecuteNonQuery(dbCommand));
		}/// <summary>
		/// LogciDelete
		...
		{
		    string sql = "update [LES].[TI_IFM_SRM_BARCODE] WITH(ROWLOCK) "
