namespace DAL.LES
{
    ...
    /// SapOutboundLogDAL
    /// </summary>
    public partial class SapOutboundLogDAL
    {
        /// <summary>
        /// MSG_CONTENT留空
        /// </summary>
        ...
        /// <returns></returns>
        public List<SapOutboundLogInfo> GetListForPage(string textWhere, string textOrder, int pageIndex, int pageRow)
        {
            ...
            string sql = "select top " + pageRow + " * from "
                ...
                "from [LES].[TI_IFM_SAP_OUTBOUND_LOG]  with(nolock) " + whereText + ") T "
                + "where rownumber > " + (pageIndex - 1) * pageRow + " ";
            Database db = DatabaseFactory.CreateDatabase();
            DbCommand cmd = db.GetSqlStringCommand(sql);
            List<SapOutboundLogInfo> list = new List<SapOutboundLogInfo>();
            using (IDataReader dr = db.ExecuteReader(cmd))
            {
                while (dr.Read())
                {
                    list.Add(CreateSapOutboundLogInfo(dr));
                }
            }
            return list;
        }
        ...
        public List<SapOutboundLogInfo> GetListForUnsend()
        {
            string sql = "select [ID],[FID],[METHOD_CODE] "
                + "from [LES].[TI_IFM_SAP_OUTBOUND_LOG] with(nolock) "
                + "where [VALID_FLAG] = 1 "
            ...
            using (IDataReader dr = db.ExecuteReader(dbCommand))
            {
                while (dr.Read())
                {
                    SapOutboundLogInfo info = new SapOutboundLogInfo();
