        ...
        case "40":///combogrid
            ...
            $('#' + tId).combogrid({
                ...
                onShowPanel: function () {
                    ...
                    if (array.get('ENTITY_NAME') == "WarehouseLocation") {
                        if (isMultiple) return;
                        var sZoneNo = $('#40-SZoneNo').combogrid('getValue');
                        var tZoneNo = $('#40-TZoneNo').combogrid('getValue');
                        var zoneNo = new Map();
                        zoneNo.put("SDloc", sZoneNo);
                        zoneNo.put("TDloc", tZoneNo);
                        for (var i = 0; i < zoneNo.key.length; i++) {
                            var subcboUrl = "";
                            for (var j = 0; j < array.key.length; j++) {
                                if (array.key[j] == "FILTER") {
                                    array.keys[j] = 'and ZONE_NO = ^' + zoneNo.keys[i] + '^';
                                }
                                subcboUrl += array.key[j] + "=" + array.keys[j];
                                if (i < array.key.length - 1)
                                    subcboUrl += "&";
                            }

                            ///控件名称
                            var ctlBindName = "#40-D-" + zoneNo.key[i];
                            var re = /-[A-Z]-/;
                            ///判断控件是否存在
                            if ($(ctlBindName).length > 0) {
                                ///获取当前控件选定值
