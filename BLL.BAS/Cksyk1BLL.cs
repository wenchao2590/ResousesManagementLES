...
namespace BLL.BAS
{
    public class Cksyk1BLL
    {
        Cksyk1DAL dal = new Cksyk1DAL();
        public List<Cksyk1Info> GetListByPage(string textWhere, string textOrder, int pageIndex, int pageRow, out int dataCount)
        {
            dataCount = dal.GetCounts(textWhere);
            return dal.GetListByPage(textWhere, textOrder, pageIndex, pageRow);
        }
        public Cksyk1Info SelectInfo(int nid)
        {
            return dal.GetInfo(nid);
        }

        public int InsertInfo(Cksyk1Info info)
        {
            return 0;
        }
