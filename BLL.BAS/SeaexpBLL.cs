...
namespace BLL.BAS
{
    public class SeaexpBLL
    {
        SeaexpDAL dal = new SeaexpDAL();
        public List<SeaexpInfo> GetListByPage(string textWhere, string textOrder, int pageIndex, int pageRow, out int dataCount)
        {
            dataCount = dal.GetCounts(textWhere);
            return dal.GetListByPage(textWhere, textOrder, pageIndex, pageRow);
        }
        public SeaexpInfo SelectInfo(int nid)
        {
        ...
        public bool UpdateInfo(List<CommonField> fields, int nid)
        {
            ...
            if (!string.IsNullOrEmpty(updatefields))
                sql += "update dbo.[dbj2] "
                    ...
                    + "where [f1] = '" + f1 + "';";
            ///DBJ1
            updatefields = CommonBLL.GetUpdateFieldSql(fields, "Dbj1");
            if (!string.IsNullOrEmpty(updatefields))
                sql += "update dbo.[dbj1] "
                    + "set " + updatefields.Substring(1) + " "
                    + "where [f1] = '" + f1 + "';";
