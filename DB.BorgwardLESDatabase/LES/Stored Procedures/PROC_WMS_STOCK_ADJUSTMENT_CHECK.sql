...
-- Description:	验证库存调整导入数据
-- =============================================
CREATE PROCEDURE [LES].[PROC_WMS_STOCK_ADJUSTMENT_CHECK]
...
BEGIN
	...
	UPDATE [LES].TE_WMS_STOCK_ADJUSTMENT_TEMP
	...
	WHERE STOCK_IDENTITY NOT IN (SELECT  STOCK_IDENTITY FROM [LES].TE_WMS_STOCK_ADJUSTMENT_TEMP M JOIN 
	...
	WHERE STOCK_IDENTITY NOT IN 
	(
		SELECT M.STOCK_IDENTITY FROM [LES].TE_WMS_STOCK_ADJUSTMENT_TEMP M 
		JOIN LES.[TM_BAS_PARTS_STOCK] Z ON M.PLANT=Z.PLANT AND M.WM_NO=Z.WM_NO AND M.ZONE_NO=Z.ZONE_NO AND M.PART_NO=Z.PART_NO 
		--AND M.SUPPLIER_NUM = Z.SUPPLIER_NUM--jinmiao 
		--AND M.PACKAGE_MODEL=Z.INBOUND_PACKAGE_MODEL--jinmiao
	)AND VALID_FLAG!=0
	
	--UPDATE TE
	--SET VALID_FLAG = 0, ERROR_MSG = ERROR_MSG + '零件批次与零件仓库中信息不一致:【'+TE.PART_NO+'】;'
	--from 
	--	[LES].TE_WMS_STOCK_ADJUSTMENT_TEMP TE,
	...
	--返回错误数据
	SELECT PART_NO,ERROR_MSG FROM [LES].TE_WMS_STOCK_ADJUSTMENT_TEMP WHERE VALID_FLAG=0
	END TRY
	BEGIN CATCH
		SELECT 
