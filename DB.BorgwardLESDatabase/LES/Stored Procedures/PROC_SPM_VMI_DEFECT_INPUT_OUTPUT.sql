...
-- Description:	不合格品VMI入库出库存储过程
-- =============================================
CREATE PROCEDURE [LES].[PROC_SPM_VMI_DEFECT_INPUT_OUTPUT]
	@RETURN_ID int,
...
BEGIN
	...
	--如果是不合格品出库，直接扣减库存，存储过程结束
	IF @ACTION_TYPE=2 AND (SELECT COUNT(1) FROM [LES].[TE_SPM_VMI_STOCKS_TEMP] WHERE VALID_FLAG=0)=0
	BEGIN
		UPDATE [LES].[TT_WMS_STOCKS] 
		SET
		STOCKS_NUM=TE.STOCKS_NUM+ISNULL(WS.STOCKS_NUM,0)
		,STOCKS=CASE WHEN WS.PACKAGE IS NULL OR WS.PACKAGE=0
					THEN 
						0
					ELSE
						CASE WHEN TE.STOCKS_NUM+ISNULL(WS.STOCKS_NUM,0)>0
						THEN
							CEILING(CAST(TE.STOCKS_NUM+ISNULL(WS.STOCKS_NUM,0) AS FLOAT)/CAST(WS.PACKAGE AS FLOAT))
						ELSE
							FLOOR(CAST(TE.STOCKS_NUM+ISNULL(WS.STOCKS_NUM,0) AS FLOAT)/CAST(WS.PACKAGE AS FLOAT))
						END
					END
		,AVAILBLE_STOCKS=TE.STOCKS_NUM+ISNULL(WS.STOCKS_NUM,0)-ISNULL(WS.FROZEN_STOCKS,0)
		,FRAGMENT_NUM=(TE.STOCKS_NUM+ISNULL(WS.STOCKS_NUM,0)-ISNULL(WS.FROZEN_STOCKS,0))%ISNULL(WS.PACKAGE,1)
		FROM [LES].[TT_WMS_STOCKS] AS WS
		INNER JOIN [LES].[TE_SPM_VMI_STOCKS_TEMP] AS TE
		ON TE.D_PLANT=WS.PLANT AND TE.D_WM_NO=WS.WM_NO AND TE.D_ZONE_NO=WS.ZONE_NO AND TE.D_DLOC=WS.DLOC AND TE.PART_NO=WS.PART_NO AND TE.SUPPLIER_NUM=WS.SUPPLIER_NUM
	...
	--如果是不合格品入库，增加或插入库存，存储过程结束
	IF @ACTION_TYPE=1 AND (SELECT COUNT(1) FROM [LES].[TE_SPM_VMI_STOCKS_TEMP] WHERE VALID_FLAG=0)=0
	BEGIN
		--更新存在的库存记录数
		UPDATE [LES].[TT_WMS_STOCKS] 
		SET
		...
		FROM 
			...
			[LES].[TM_BAS_PARTS_STOCK] PS with(nolock)
		WHERE 
			TE.D_PLANT = PS.PLANT
	...
	--最后将不合格品出库入库明细数据插入VMI库存交易明细表
	IF (SELECT COUNT(1) FROM [LES].[TE_SPM_VMI_STOCKS_TEMP] WHERE VALID_FLAG=0)=0
	BEGIN
		declare @MODIFICATION_CODE NVARCHAR(50)
	
		SELECT  @MODIFICATION_CODE=CASE WHEN @ACTION_TYPE=1 THEN N'DefectInput' ELSE N'DefectOuptput' END;
		...
		SELECT 
			...
			PS.PACKAGE,
			TE.STOCKS_NUM,
			CASE WHEN PS.PACKAGE IS NULL OR PS.PACKAGE=0
						THEN 
							0
						ELSE
							CASE WHEN TE.STOCKS_NUM>0
							THEN
								CEILING(CAST(TE.STOCKS_NUM AS FLOAT)/CAST(PS.PACKAGE AS FLOAT))
							ELSE
								FLOOR(CAST(TE.STOCKS_NUM AS FLOAT)/CAST(PS.PACKAGE AS FLOAT))
							END
						END
			,@MODIFICATION_CODE
			,TE.SUPPLIER_NUM
		...
		ON TE.D_PLANT=PS.PLANT AND TE.D_WM_NO=PS.WM_NO AND TE.D_ZONE_NO=PS.ZONE_NO AND TE.D_DLOC=PS.DLOC AND TE.SUPPLIER_NUM=PS.SUPPLIER_NUM AND TE.PART_NO=PS.PART_NO
	END
END
