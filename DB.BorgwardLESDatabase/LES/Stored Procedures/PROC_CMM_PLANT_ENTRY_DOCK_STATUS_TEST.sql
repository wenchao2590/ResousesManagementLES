...
-- EXEC [LES].[PROC_CMM_PLANT_ENTRY_DOCK_STATUS_TEST] '474BBCAB-CBDB-417E-A6C5-82B4B67595CE','ADMIN'
-- =============================================
CREATE PROCEDURE [LES].[PROC_CMM_PLANT_ENTRY_DOCK_STATUS_TEST]
...
BEGIN
	...
	BEGIN TRAN
		...
		BEGIN
			...
			--根据车牌号找到入场时存入的门号
			SELECT TOP 1 @DOOR_NO=DOOR_NO FROM LES.TI_MID_RFID_PLANT_ENTRY WHERE CARGO_LICENCE = @NEXT_VEHICLE_NO ORDER BY ID DESC
			...
			BEGIN
				...
				VALUES(@FID_DOCK_HOLD_MESSAGE,@NEXT_VEHICLE_NO,@NEXT_SHEET_NO,N'请司机前往道口【'+ISNULL(@DOCK,N'')+N'】开始卸货',GETDATE(),1,@DOOR_NO);
