...
-- =============================================
CREATE PROCEDURE [LES].[PROC_WMM_REFILL_PLAN_CREATE]
	@R_PLANT NVARCHAR(5),
...
BEGIN
	...
	BEGIN TRY
		SET @REPACKAGE_MINUTES = (SELECT CAST([PARAMETER_VALUE] AS INT) FROM [LES].[TS_SYS_CONFIG] WHERE PARAMETER_NAME='RefillEndTime')
	END TRY
	BEGIN CATCH
	...
	DECLARE ZONE_CURS1 CURSOR FOR
		SELECT PLANT,WM_NO,ZONE_NO FROM [LES].[TT_WMM_REFILL_REQUEST]
	...
	BEGIN	--存储区
		...
		BEGIN TRY
			...
			INSERT INTO @ProcessTable (ID)
				SELECT ID FROM [LES].[TT_WMM_REFILL_REQUEST]
			...
			SET  @REPACKAGE_NO = 'BH'+CONVERT(VARCHAR(2), GETDATE(),2)+REPLICATE('0',8-LEN(@SEQ))+RTRIM(@SEQ)
			...
			INSERT INTO [LES].[TT_WMM_REPACKAGE_HEAD] (
				...
				SELECT
					...
					,@WM_NO					--,[WM_NO]
					,@ZONE_NO				--,[ZONE_NO]
					,2					    --,[REPACKAGE_TYPE]   1：翻包计划； 2：补货计划
			...
			INSERT INTO [LES].[TT_WMM_REPACKAGE_DETAIL] (
				...
				SELECT
						...
						,NULL						--,[LOCATION]		
						,NULL						--,[PART_TYPE]		
						,REQUIRED_BOX_NUM			--,[NUM]		
						,PACKAGE_MODEL				--,[INHOUSE_PACKAGE_MODEL]		
						,PACKAGE					--,[INHOUSE_PACKAGE]		
						...
						,NULL						--,[UPDATE_DATE]		
						,NULL						--,[ACTUAL_QTY]		
						,PACKAGE*REQUIRED_BOX_NUM	--,[REQUIRED_QTY]
					FROM (
								
						SELECT R.PLANT,R.WM_NO,R.ZONE_NO,R.PART_NO,R.PART_CNAME,R.PART_NICKNAME,R.DLOC,R.PACKAGE_MODEL,SUM(R.REQUIRED_BOX_NUM) REQUIRED_BOX_NUM,R.PACKAGE
							FROM [LES].[TT_WMM_REFILL_REQUEST] R INNER JOIN @ProcessTable P ON R.ID=P.ID
							--WHERE R.REQUIRED_BOX_NUM>0  and R.PACKAGE>0
							GROUP BY R.PLANT,R.WM_NO,R.ZONE_NO,R.PART_NO,R.PART_CNAME,R.PART_NICKNAME,R.DLOC,R.PACKAGE_MODEL,R.PACKAGE
			...
			UPDATE R SET
						R.DEAL_FLAG=1
					,R.DEAL_DATE=GETDATE()
					,R.REFILL_NO=@REPACKAGE_NO
					,R.UPDATE_USER='admin'
					,R.UPDATE_DATE=GETDATE()
				FROM [LES].[TT_WMM_REFILL_REQUEST] R
