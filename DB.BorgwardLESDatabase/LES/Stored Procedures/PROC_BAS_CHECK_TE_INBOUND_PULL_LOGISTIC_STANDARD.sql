...
/*   Program Name:  PROC_BAS_CHECK_TE_INBOUND_PULL_LOGISTIC_STANDARD                          */
/*   Called By:     by the Page							*/
/*   Purpose:       This is the main stored procedure for the       */
/*   author:       wangchanghong	2011-06-20   				       */
/********************************************************************/
CREATE PROC [LES].[PROC_BAS_CHECK_TE_INBOUND_PULL_LOGISTIC_STANDARD]
...
AS
	...
	UPDATE [LES].TE_BAS_TMP_INBOUND_PULL_LOGISTIC_STANDARD
	SET VALID_FLAG = 0,
	ERROR_MSG = ERROR_MSG + '主键重复;'
	WHERE 
	BATCH_NO =@batch_no  AND
	LOGICAL_PK IN(
	SELECT LOGICAL_PK  FROM [LES].TE_BAS_TMP_INBOUND_PULL_LOGISTIC_STANDARD WHERE BATCH_NO =@batch_no  GROUP BY LOGICAL_PK HAVING COUNT(1) > 1)
	...
	BATCH_NO <> @batch_no AND
	LOGICAL_PK IN(
	SELECT LOGICAL_PK FROM [LES].TE_BAS_TMP_INBOUND_PULL_LOGISTIC_STANDARD WHERE BATCH_NO = @batch_no  AND VALID_FLAG IS NULL)
	...
	WHERE 	LOGICAL_PK IN(
	SELECT LOGICAL_PK FROM [LES].TE_BAS_TMP_INBOUND_PULL_LOGISTIC_STANDARD AS INBOUND
