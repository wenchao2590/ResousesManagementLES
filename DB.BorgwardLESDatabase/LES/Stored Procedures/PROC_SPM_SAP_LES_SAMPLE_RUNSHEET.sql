...
CREATE PROCEDURE [LES].[PROC_SPM_SAP_LES_SAMPLE_RUNSHEET]
AS
BEGIN
Set    NOCOUNT    ON;
Set XACT_ABORT ON;
...
Begin Try
	...
	Select TOP 500 SEQ_ID,VBELN
	  From (SELECT SEQ_ID=MAX(SEQ_ID),VBELN  --目前调整为每次处理500个订单
			  FROM LES.TI_SPM_DELIVERY_RUNSHEET_IN 
	...
	OPEN RUNSHEET_CUR
	FETCH  NEXT  FROM RUNSHEET_CUR INTO @VBELN,@POSNR,@LIFNR,@LFUHR,@WERKS,@EMLIF,@ZDOCK,@DISPO,@LFART,@LESTYPE,@ZVERSION,@STRAS,@LFDAT,@LFUHR,@MEINS,@LGORT,@STRAS_FROM
	WHILE( @@fetch_status = 0 )
	BEGIN
	Print('开始处理订单' + @VBELN)
		DECLARE @recordenum INT--记录数
		DECLARE @InsertId BIGINT;
		SELECT @recordenum=COUNT(*) FROM LES.TT_SPM_SAMPLE_PURCHASE_RUNSHEET WHERE PLAN_RUNSHEET_NO=@VBELN --AND	SUPPLIER_NUM=@LIFNR AND DOCK=@ZDOCK
		...
		BEGIN
			--添加主表信息
   INSERT INTO LES.TT_SPM_SAMPLE_PURCHASE_RUNSHEET(PLAN_RUNSHEET_NO
			                                        ,PLAN_NO
													,SUPPLIER_NUM
			...
			SELECT @InsertId= SCOPE_IDENTITY()
			 ...
			 --添加明细信息		 
		 INSERT INTO LES.TT_SPM_SAMPLE_PURCHASE_RUNSHEET_DETAIL
						([PLAN_RUNSHEET_SN]
						,PLANT
						,SUPPLIER_NUM
						,PART_NO
...
Begin Catch
	...
	insert into [LES].[TS_SYS_EXCEPTION] (time_stamp, [application], [METHOD], class,  exception_message, error_code)
		select getdate(),'INTERFACE','PROC_SPM_SAP_LES_SAMPLE_RUNSHEET','Procedure',ISNULL(error_message(),'')+';VBELN:'+IsNull(@VBELN,''),ERROR_LINE()
