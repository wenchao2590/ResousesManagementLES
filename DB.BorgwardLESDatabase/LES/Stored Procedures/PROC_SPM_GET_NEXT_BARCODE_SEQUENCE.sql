--生成下一个条码的流水号
CREATE PROCEDURE [LES].[PROC_SPM_GET_NEXT_BARCODE_SEQUENCE]
(    
...
BEGIN
    ...
    UPDATE  LES.TS_PLN_BARCODE_SEQUENCE WITH(ROWLOCK)
    ...
    IF @@ROWCOUNT = 0
    BEGIN
        INSERT INTO LES.TS_PLN_BARCODE_SEQUENCE(SEQUENCE_NAME,CURRENT_VALUE,INIT_VALUE,MAX_VALUE,STEP_VALUE, LAST_UPDATE_DATE)
    ...
    FROM    LES.TS_PLN_BARCODE_SEQUENCE
    WHERE   UPPER(RTRIM(LTRIM(SEQUENCE_NAME))) = UPPER(RTRIM(LTRIM(@SequenceName)))
