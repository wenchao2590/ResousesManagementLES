...
CREATE PROCEDURE [LES].[PROC_EPS_GET_NEXT_SEQUENCE]
(    
...
BEGIN
    ...
    BEGIN TRY
        BEGIN TRAN
            UPDATE  LES.TS_EPS_SEQUENCE WITH(ROWLOCK)
            ...
            IF @@ROWCOUNT = 0
            BEGIN
                INSERT INTO LES.TS_EPS_SEQUENCE(SEQUENCE_NAME,CURRENT_VALUE,INIT_VALUE,MAX_VALUE,STEP_VALUE, LAST_UPDATE_DATE)
                VALUES(@SequenceName, 1, 1, 99999999, 1, GETDATE())                
            END
            
            SELECT  @NextSequence = ISNULL(PREFIX_STRING,'') + CURRENT_VALUE + ISNULL(POSTFIX_STRING,'')
            FROM    LES.TS_EPS_SEQUENCE
            WHERE   UPPER(RTRIM(LTRIM(SEQUENCE_NAME))) = UPPER(RTRIM(LTRIM(@SequenceName)))
