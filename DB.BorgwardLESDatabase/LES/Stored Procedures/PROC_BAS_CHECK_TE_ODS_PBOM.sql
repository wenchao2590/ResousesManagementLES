...
/********************************************************************/
CREATE PROC [LES].[PROC_BAS_CHECK_TE_ODS_PBOM]
...
AS
	...
	UPDATE [LES].TE_ODS_TEMP_PBOM
	SET VALID_FLAG = 0,
	ERROR_MSG = ERROR_MSG + '主键重复;'
	WHERE 
	BATCH_NO =@batch_no  AND
	LOGICAL_PK IN(
	SELECT LOGICAL_PK  FROM [LES].TE_ODS_TEMP_PBOM WHERE BATCH_NO =@batch_no  GROUP BY LOGICAL_PK HAVING COUNT(1) > 1)
	...
	BATCH_NO <> @batch_no AND
	LOGICAL_PK IN(
	SELECT LOGICAL_PK FROM [LES].TE_ODS_TEMP_PBOM WHERE BATCH_NO = @batch_no  AND VALID_FLAG IS NULL)
