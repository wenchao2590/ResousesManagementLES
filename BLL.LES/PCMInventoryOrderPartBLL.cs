...
namespace BLL.LES
{
    public class PCMInventoryOrderPartBLL
    {
        #region Common
        PCMInventoryOrderPartDAL dal = new PCMInventoryOrderPartDAL();
        /// <summary>
        /// 分页获取集合
        /// </summary>
        ...
        /// <returns>List<PCMInventoryOrderPartInfo></returns>
        public List<PCMInventoryOrderPartInfo> GetListByPage(string textWhere, string textOrder, int pageIndex, int pageRow, out int dataCount)
        {
            dataCount = dal.GetCounts(textWhere);
            return dal.GetListByPage(textWhere, textOrder, pageIndex, pageRow);
        }

        public PCMInventoryOrderPartInfo SelectInfo(long id)
        {
        ...
        public bool ReleaseInfos(List<string> rowsKeyValues, string loginUser)
        {
            ...
            foreach (var item in rowsKeyValues)
            {
                ...
                if (values.Length == 2)
                    sql += " update[LES].[TT_PCM_INVENTORY_ORDER_PART] WITH(ROWLOCK) set [COMMENTS] = N'" + values[1] + "',[MODIFY_USER] = N'" + loginUser + "' ,[MODIFY_DATE] = GETDATE() where [ID] = " + values[0] + ";";
