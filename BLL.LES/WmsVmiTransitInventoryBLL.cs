...
namespace BLL.LES
{
    public class WmsVmiTransitInventoryBLL
    {
        #region Common
        WmsVmiTransitInventoryDAL dal = new WmsVmiTransitInventoryDAL();
        /// <summary>
        /// 分页获取集合
        /// </summary>
        ...
        /// <returns>List<WmsVmiTransitInventoryInfo></returns>
        public List<WmsVmiTransitInventoryInfo> GetListByPage(string textWhere, string textOrder, int pageIndex, int pageRow, out int dataCount)
        {
            dataCount = dal.GetCounts(textWhere);
            return dal.GetListByPage(textWhere, textOrder, pageIndex, pageRow);
        }

        public WmsVmiTransitInventoryInfo SelectInfo(long id)
        {
            return dal.GetInfo(id);
        }

        public long InsertInfo(WmsVmiTransitInventoryInfo info)
        {
            return dal.Add(info);
        }
        ...
        {
            List<WmsVmiTransitInventoryInfo> wmsVmiTransitInventories = dal.GetList("[ID] in (" + string.Join(",", rowsKeyValues.ToArray()) + ") and [PROCESS_FLAG] = " + (int)ProcessFlagConstants.Untreated, string.Empty);
            if (wmsVmiTransitInventories.Count != rowsKeyValues.Count())
                throw new Exception("MC:0x00000523");///仅同步状态为待处理时可以更新已取消
            string sql = "update [LES].[TI_IFM_WMS_VMI_TRANSIT_INVENTORY] WITH(ROWLOCK) set [PROCESS_FLAG] = " + (int)ProcessFlagConstants.Cancel + ",[MODIFY_DATE] = GETDATE(),[MODIFY_USER] = N'" + loginUser + "' where [ID] in (" + string.Join(",", rowsKeyValues.ToArray()) + ")";
