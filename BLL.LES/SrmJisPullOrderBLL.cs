...
namespace BLL.LES
{
    public class SrmJisPullOrderBLL
    {
        #region Common
        SrmJisPullOrderDAL dal = new SrmJisPullOrderDAL();
        /// <summary>
        /// 分页获取集合
        /// </summary>
        ...
        /// <returns>List<SrmJisPullOrderInfo></returns>
        public List<SrmJisPullOrderInfo> GetListByPage(string textWhere, string textOrder, int pageIndex, int pageRow, out int dataCount)
        {
        ...
        /// <returns></returns>
        public SrmJisPullOrderInfo SelectInfo(long id)
        {
        ...
        /// <param name="textOrder">string 排序语句,无须order by</param>
        /// <returns>List<SrmJisPullOrderInfo></returns>
        public List<SrmJisPullOrderInfo> GetList(string textWhere, string textOrder)
        {
        ...
        {
            List<SrmJisPullOrderInfo> srmJisPullOrders = dal.GetList("[ID] in (" + string.Join(",", rowsKeyValues.ToArray()) + ") and [PROCESS_FLAG] = " + (int)ProcessFlagConstants.Untreated, string.Empty);
            if (srmJisPullOrders.Count != rowsKeyValues.Count())
                throw new Exception("MC:0x00000523");///仅同步状态为待处理时可以更新已取消
            string sql = "update [LES].[TI_IFM_SRM_JIS_PULL_ORDER] WITH(ROWLOCK) set [PROCESS_FLAG] = " + (int)ProcessFlagConstants.Cancel + ",[MODIFY_DATE] = GETDATE(),[MODIFY_USER] = N'" + loginUser + "' where [ID] in (" + string.Join(",", rowsKeyValues.ToArray()) + ")";
        ...
        {
            List<SrmJisPullOrderInfo> srmJisPullOrders = dal.GetList("[ID] in (" + string.Join(",", rowsKeyValues.ToArray()) + ") and [PROCESS_FLAG] = " + (int)ProcessFlagConstants.Suspend, string.Empty);
            if (srmJisPullOrders.Count != rowsKeyValues.Count())
            ...
            string sql = "update [LES].[TI_IFM_SRM_JIS_PULL_ORDER] WITH(ROWLOCK) set [PROCESS_FLAG] = " + (int)ProcessFlagConstants.Resend + ",[MODIFY_DATE] = GETDATE(),[MODIFY_USER] = N'" + loginUser + "' where [ID] in (" + string.Join(",", rowsKeyValues.ToArray()) + ")";
            return CommonBLL.ExecuteNonQueryBySql(sql);
        }
        #region Interface
        /// <summary>
