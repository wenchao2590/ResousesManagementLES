...
namespace BLL.LES
{
    public class MesProductionlineProcessOrderBLL
    {
        #region Common
        MesProductionlineProcessOrderDAL dal = new MesProductionlineProcessOrderDAL();
        /// <summary>
        /// 分页获取集合
        /// </summary>
        ...
        /// <returns>List<MesProductionlineProcessOrderInfo></returns>
        public List<MesProductionlineProcessOrderInfo> GetListByPage(string textWhere, string textOrder, int pageIndex, int pageRow, out int dataCount)
        {
            dataCount = dal.GetCounts(textWhere);
            return dal.GetListByPage(textWhere, textOrder, pageIndex, pageRow);
        }

        public MesProductionlineProcessOrderInfo SelectInfo(long id)
        {
        ...
        public long InsertInfo(MesProductionlineProcessOrderInfo info)
        {
            return dal.Add(info);
        }
        ...
        {
            List<MesProductionlineProcessOrderInfo> mesProductionlines = dal.GetList("[ID] in (" + string.Join(",", rowsKeyValues.ToArray()) + ") and [PROCESS_FLAG] = " + (int)ProcessFlagConstants.Suspend, string.Empty);
            if (mesProductionlines.Count != rowsKeyValues.Count())
            ...
            string sql = "update [LES].[TI_IFM_MES_PRODUCTIONLINE_PROCESS_ORDER] WITH(ROWLOCK) set [PROCESS_FLAG] = " + (int)ProcessFlagConstants.Resend + ",[MODIFY_DATE] = GETDATE(),[MODIFY_USER] = N'" + loginUser + "' where [ID] in (" + string.Join(",", rowsKeyValues.ToArray()) + ")";
            return CommonBLL.ExecuteNonQueryBySql(sql);
        }
    }
}
