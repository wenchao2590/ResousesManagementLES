...
namespace BLL.LES
{
    public class WmsVmiSupplyPlanBLL
    {
        #region Common
        WmsVmiSupplyPlanDAL dal = new WmsVmiSupplyPlanDAL();
        /// <summary>
        /// 分页获取集合
        /// </summary>
        ...
        /// <returns>List<WmsVmiSupplyPlanInfo></returns>
        public List<WmsVmiSupplyPlanInfo> GetListByPage(string textWhere, string textOrder, int pageIndex, int pageRow, out int dataCount)
        {
            dataCount = dal.GetCounts(textWhere);
            return dal.GetListByPage(textWhere, textOrder, pageIndex, pageRow);
        }

        public WmsVmiSupplyPlanInfo SelectInfo(long id)
        {
        ...
        public long InsertInfo(WmsVmiSupplyPlanInfo info)
        {
            return dal.Add(info);
        }
        ...
        {
            List<WmsVmiSupplyPlanInfo> wmsVmiSupplyPlans = dal.GetList("[ID] in (" + string.Join(",", rowsKeyValues.ToArray()) + ") and [PROCESS_FLAG] = " + (int)ProcessFlagConstants.Suspend, string.Empty);
            if (wmsVmiSupplyPlans.Count != rowsKeyValues.Count())
            ...
            string sql = "update [LES].[TI_IFM_WMS_VMI_SUPPLY_PLAN] WITH(ROWLOCK) set [PROCESS_FLAG] = " + (int)ProcessFlagConstants.Resend + ",[MODIFY_DATE] = GETDATE(),[MODIFY_USER] = N'" + loginUser + "' where [ID] in (" + string.Join(",", rowsKeyValues.ToArray()) + ")";
            return CommonBLL.ExecuteNonQueryBySql(sql);
        }
    }
}
