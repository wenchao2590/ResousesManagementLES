...
namespace BLL.LES
{
    public class SapMaterialReservationBLL
    {
        #region Common
        SapMaterialReservationDAL dal = new SapMaterialReservationDAL();
        /// <summary>
        /// 分页获取集合
        /// </summary>
        ...
        /// <returns>List<SapMaterialReservationInfo></returns>
        public List<SapMaterialReservationInfo> GetListByPage(string textWhere, string textOrder, int pageIndex, int pageRow, out int dataCount)
        {
            dataCount = dal.GetCounts(textWhere);
            return dal.GetListByPage(textWhere, textOrder, pageIndex, pageRow);
        }

        public SapMaterialReservationInfo SelectInfo(long id)
        {
            return dal.GetInfo(id);
        }

        public long InsertInfo(SapMaterialReservationInfo info)
        {
            return dal.Add(info);
        }
        ...
        {
            List<SapMaterialReservationInfo> sapMaterials = dal.GetList("[ID] in (" + string.Join(",", rowsKeyValues.ToArray()) + ") and [PROCESS_FLAG] = " + (int)ProcessFlagConstants.Suspend, string.Empty);
            if (sapMaterials.Count != rowsKeyValues.Count())
            ...
            string sql = "update [LES].[TI_IFM_SAP_MATERIAL_RESERVATION] WITH(ROWLOCK) set [PROCESS_FLAG] = " + (int)ProcessFlagConstants.Resend + ",[MODIFY_DATE] = GETDATE(),[MODIFY_USER] = N'" + loginUser + "' where [ID] in (" + string.Join(",", rowsKeyValues.ToArray()) + ")";
            return CommonBLL.ExecuteNonQueryBySql(sql);
        }
